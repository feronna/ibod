...
<p align="right"><?= Html::a('Back', ['cb-lkk/a-view-rating?i=' . $model->reportID . '&id=' . $model->icno], ['class' => 'btn btn-primary btn-sm'])
?></p>
...
<td class="text-center">
                                <p align="center">
                                    <?php
                                    $dean3 = \app\models\cbelajar\LkkDean::find()->where(['icno' => $model->icno, 'dokumen' => $dok->id])->orderBy(['created_dt' => SORT_DESC])->one();
//var_dump($dean->namafile);die;
                                    if (\app\models\cbelajar\LkkDean::find()->where(['icno' => $model->icno, 'dokumen' => $dok->id])->orderBy(['created_dt' => SORT_DESC])->one()) {
                                        if ($dean3->namafile) {
                                            echo'<a class="form-control" style="background-color: transparent;border:0;box-shadow: none;" 
      href=" ' . Url::to(Yii::$app->FileManager->DisplayFile($dean3->namafile), true) . '" target="_blank" >
      <i class="fa fa-download"></i> <strong><small><u> Download  </u></small></strong></a>';
                                        }
                                    }
                                    echo '<br>' . $dean3->comment;
                                    ?></p></td>
...
<p align="right"> 
    ...
    <?= Html::a('Next', ['lkk/pengesahan-admin?id=' . $model->reportID . '&icno=' . $model->icno], ['class' => 'btn btn-primary btn-sm'])
    ?></p>
    <?php ActiveForm::end(); ?>
