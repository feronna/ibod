...
<div class="row">
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
            <div>
                <p align="right">  <?= Html::a('Kembali', ['/cutibelajar/senarai-staff-fakulti'], ['class' => 'btn btn-primary btn-sm']) ?></p>
            </div>
            <div class="x_title">
...
//        foreach ($b as $b) {
        ...
        ?> 
                                ...
                                <tr> 
                                    ...
                                    <td style="width:20%">
                                        ...
                                        <?php
                                        if (($pengajian->t->MajorCd == NULL) && ($pengajian->t->MajorMinor != NULL)) {
    ...
    <?php if (!$pengajian->l) {
        ?> 
                                    ...
                                    <td style="width:20%">
                                    <?php echo strtoupper($pengajian->nama_penyelia); ?></td><?php } ?></tr>
                            <tr> 
    <?php if (!$pengajian->l) {
        ?> 
                                    <th style="width:10%" align="right">EMEL PENYELIA</th>
                                    <td style="width:20%">
                                    <?php echo ($pengajian->emel_penyelia); ?></td><?php } ?></tr>
    ...
    echo '&#10008;'; }?></td>
                    ...
                    <table class="table table-sm table-bordered jambo_table table-striped"> <tr>
                            ...
                            <?php
                            if (($pengajian->HighestEduLevelCd === 1) || ($pengajian->HighestEduLevelCd === 99) || ($pengajian->HighestEduLevelCd === 202) || ($pengajian->HighestEduLevelCd === 205) ||
                                    ($pengajian->HighestEduLevelCd === 11) || ($pengajian->HighestEduLevelCd === 101) ||
                                    ($pengajian->HighestEduLevelCd === 200) ) {
                                ?>
        <div class="row">
                            ...
                            <table class="table table-striped table-sm jambo_table table-bordered" style="text-align:center;">
                                <thead style="background-color:lightseagreen;color:white">
                                    <tr class="headings">
                                       ...
                                       <th class="column-title text-center">STATUS PENYELIA</th>
                                        <th class="column-title text-center">STATUS BSM</th>
                                        <th class="column-title text-center">PRESTASI KESELURUHAN</th>
...
//                                 $lkk = app\models\cbelajar\TblLkk::find()->where(['icno'=> $id, 'reportID'=> $i, 'status_form'=>2])->one();
                                        ...
                                        ?>
                                            ...
                                            <td>
                            <?php 
                      $c = \app\models\cbelajar\Rating::find()->where(['idLkk' => $lkk->reportID,'idKriteria'=>5])->one();
                     $b = \app\models\cbelajar\Rating::find()->where(['idLkk' => $lkk->reportID,'idKriteria'=>7])->one();
                     $a = \app\models\cbelajar\Rating::find()->where(['idLkk' => $lkk->reportID,'idKriteria'=>6])->one();
                     $d = \app\models\cbelajar\Rating::find()->where(['idLkk' => $lkk->reportID,'idKriteria'=>4])->one();
                     $e = \app\models\cbelajar\Rating::find()->where(['idLkk' => $lkk->reportID,'idKriteria'=>3])->one();
                     $f = \app\models\cbelajar\Rating::find()->where(['idLkk' => $lkk->reportID,'idKriteria'=>2])->one();
                     $g = \app\models\cbelajar\Rating::find()->where(['idLkk' => $lkk->reportID,'idKriteria'=>1])->one();
                     $total = ($a->p_komen + $b->p_komen + $c->p_komen + $d->p_komen
                                  + $e->p_komen + $f->p_komen + $g->p_komen);                                
                    if($lkk->status_r =="DONE" && $lkk->status_jfpiu == "Diperakukan")
                                 {?>
...
<td>
                                        <?php if ($lkk->status_bsm == "Admin Manually Upload") {
                                            ?>
            <?= Html::a('<i class="fa fa-file-pdf-o" aria-hidden="true"></i>', ['lkk/lihat-borang-kj', "id" => $lkk->reportID], ['class' => 'btn btn-default btn-xs'])
            ?>
                                                <?php } 
                                                
                                          elseif($lkk->agree == 1){?>
                                                    
                                                <?= Html::a('<i class="fa fa-bar-chart" aria-hidden="true"></i>', ['tindakan-kj', "i" => $lkk->reportID], ['class' => 'btn btn-default btn-xs'])?>
                                      <?php          } else{
                                          echo '-';
                                      }
?>
...
//                                 $lkk = app\models\cbelajar\TblLkk::find()->where(['icno'=> $id, 'reportID'=> $i, 'status_form'=>2])->one();
        ...
        <?php if ($lkk->status_bsm == "Admin Manually Upload") {
            ?>
            <?= Html::a('<i class="fa fa-file-pdf-o" aria-hidden="true"></i>', ['lkk/lihat-borang', "id" => $lkk->reportID], ['class' => 'btn btn-default btn-xs'])
            ?>
                                        <?php } ?>

                                            </td>
                                            <td class="text-center" style="width:30%;">
                                        <?= Html::a('<i class="fa fa-bar-chart" aria-hidden="true"></i>', ['lkk/tindakan-kj', "i" => $lkk->reportID], ['class' => 'btn btn-default btn-xs'])
                                        ?> 
