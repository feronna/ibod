...
<table class="table table-bordered table-condensed table-striped table-sm jambo_table">
    ...
    <tr>
        ...
        <thead>
            <th class="column-title text-right" colspan="4">There are <?= $total ?> Staff That Taken Leave Until <?= TblRecords::viewBulan($month)?> <?= $year ?> </th>
        </thead>
    </tr>
    ...
    <?php foreach ($staff_list as $data) { ?>
        ...
        <?php foreach (TblRecords::getRecord($data->ICNO, "$year", "$mth") as $test) { ?>
            <tr>
