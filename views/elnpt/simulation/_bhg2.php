            ...
            <tr>
                ...
                <td class="text-center">
                    <?= ($input[$index]->tahap_penyeliaan >= 6) ? Html::a("<i class='fa fa-file ' aria-hidden='true'></i>
                        ", Url::to(Yii::$app->FileManager->DisplayFile($input[$index]->fileHash($input[$index]->lpp_id, 2, $input[$index]->id)), true), ['target' => '_blank', 'class' => 'btn btn-xs btn-default']) : '' . '-' ?></td>
                <td class="text-center">
...
<div class="form-group">
    <label class="control-label col-md-3 col-sm-3 col-xs-12">Jumlah bilangan perjumpaan dengan semua pelajar seliaan pascasiswazah / prasiswazah pada Semester 1 / 2</label>
    <div class="col-md-3 col-sm-3 col-xs-12">
        <?= $form->field($input[1], "[1]bil_pelajar")->textInput(['type' => 'number', 'placeholder' => "0", 'style' => "text-align:center;",  'disabled' => ($lpp->PYD == Yii::$app->user->identity->ICNO ? $check : true)])->label(false) ?>
    </div>
</div>

<div style="clear: both;" class="form-group pull-right">
    <?= (!$check && $lpp->PYD == \Yii::$app->user->identity->ICNO) ? Html::submitButton('Simpan', ['class' => 'btn btn-primary', 'data' => ['disabled-text' => 'Please wait..']]) : '' ?>
</div>
