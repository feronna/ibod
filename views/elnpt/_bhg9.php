...
<div class="table-responsive">
  ...
  <dd><i>*Taken from STARS Attendance Report</i></dd>
        ...
        <?php if(
                (($lpp->PPP == Yii::$app->user->identity->ICNO AND (date('Y-m-d H:i:s') <= $tahun->penilaian_PPP_tamat) AND ($lpp->PPP_sah == 0)) OR ($lpp->PPP == \Yii::$app->user->identity->ICNO  AND (is_null($req) ? null : $req->ICNO == Yii::$app->user->identity->ICNO))) 
                OR (($lpp->PPK == Yii::$app->user->identity->ICNO AND (date('Y-m-d H:i:s') <= $tahun->penilaian_PPK_tamat) AND ($lpp->PPK_sah == 0)) OR ($lpp->PPK == \Yii::$app->user->identity->ICNO  AND (is_null($req) ? null : $req->ICNO == Yii::$app->user->identity->ICNO))) 
                OR (($lpp->PEER == Yii::$app->user->identity->ICNO AND (date('Y-m-d H:i:s') <= $tahun->penilaian_PEER_tamat) AND ($lpp->PEER_sah == 0)) OR ($lpp->PEER == \Yii::$app->user->identity->ICNO  AND (is_null($req) ? null : $req->ICNO == Yii::$app->user->identity->ICNO)))
                ) { ?>
