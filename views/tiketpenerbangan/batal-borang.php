...
<h5> <strong><center>BATAL PERMOHONAN TIKET PENERBANGAN</center></strong> </h5>
<div> 
    <div class="x_panel"> 
        ...
        <div class="form-group"> 
                ...
                <?=
                    $form->field($model, 'status_borang')->widget(Select2::classname(), [
                        //'data' => ArrayHelper::map(RefAkses::find()->orderBy(['akses_id' => SORT_ASC])->all(), 'akses_id', 'akses_label'),
                        'data' => ['Selesai Permohonan' => 'MEMOHON', 'BATAL' => 'BATAL PERMOHONAN'],
                        'hideSearch' => true,
                        'options' => [
...
<!--                <= Html::a('</i> Kembali', ['view', 'id'=>$model->ICNO], ['class'=>'btn btn-primary']) ?>       -->
                <?= Html::submitButton('<i class="fa fa-lock-open"></i> Batal', ['class' => 'btn btn-success', 'data'=>['disabled-text' => 'Sila Tunggu..']]) ?>
            </div>
        </div>
