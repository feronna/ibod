...
<div class="row">
    <div class="col-xs-12 col-md-12 col-lg-12">
        ...
        <div class="x_panel">
            <div class="x_title">
                <h2><strong>Job Description (JD)</strong></h2> 
                <div class="clearfix"></div>
            </div>

            <?php
            if ($deskripsi) {
                $ikhtisas = \app\models\myportfolio\TblIkhtisas::find()->where(['portfolio_id' => $deskripsi->id])->all();
                $lihatDimensi = \app\models\myportfolio\TblDimensi::find()->where(['portfolio_id' => $deskripsi->id])->all();
                $pengalaman = \app\models\myportfolio\TblPengalaman::find()->where(['portfolio_id' => $deskripsi->id])->all();
                $lihatKompetensi = \app\models\myportfolio\TblKompetensi::find()->where(['portfolio_id' => $deskripsi->id])->all();
                $akauntabiliti = \app\models\myportfolio\TblAkauntabiliti::find()->where(['portfolio_id' => $deskripsi->id])->all();
                $tugas = \app\models\myportfolio\TblTugasUtama::find()->where(['akauntabiliti_id' => $deskripsi->id])->all();
                $syarat = \app\models\myportfolio\TblSyaratTambahan::find()->where(['portfolio_id' => $deskripsi->id])->all();
                ?>
                    ...
                    <div class="table-responsive">
                        <table class="table table-sm table-bordered">
                           ...
                           <tr>
                            ...
                            <tr> <td colspan="6" style="text-align:justify"><?php echo $deskripsi->kata_kerja ?> <?php echo $deskripsi->object ?>  <?php echo $deskripsi->tujuan ?></td></tr>
                            ...
                            if ($pengalaman) {
                                ...
                                foreach ($pengalaman as $key => $item) {
                                    ...
                                    <tr>
                                        <td width="50px" align="center"><?= $key + 1 ?></td>
                                        <td colspan="4">
                                            <?= $item->tempoh ?> <?= $item->pengalaman ?></td>
                                    </tr>
                ...
                echo "<br/>Pemohon belum melengkapkan/menghantar JD.";
            }
            ?>
        </div>
    </div> 
