            ...
            GridView::widget([
                ...
                'columns' => [
                    ...
                    [
                        'label' => 'Jemputan',
                        'value' => function($model) {
                            return Html::a('<i class="fa fa-download"></i>', ['jemputan', 'iklan_id' => $model->iklan_id, 'title' => 'iv'], ['class' => 'btn btn-default btn-sm', 'target' => '_blank']);
                        },
                                'format' => 'raw',
                                ...
                                'value' => function($model) {
                                    if ($model->status_kehadiran_id != 0) {
                                        if ($model->iklan->status_skype_iv == 1) {
                                            return '<span class="label label-' . $model->kehadiranSkypeIv->label . '">' . $model->kehadiranSkypeIv->name . '</span>';
                                        } else {
                                            return '<span class="label label-' . $model->kehadiranIv->label . '">' . $model->kehadiranIv->name . '</span>';
                                        }
                                    } else {
                                        return Html::button('<i class="fa fa-edit"></i>', ['id' => 'modalButton', 'value' => \yii\helpers\Url::to(['kehadiran', 'id' => $model->id, 'title' => 'iv']), 'class' => 'btn btn-default btn-sm mapBtn']);
                                    }
                                },
                                        'format' => 'raw',
                                        'contentOptions' => ['class' => 'text-center'],
                                    ...
                                    [
                                        'label' => 'Status Temuduga',
                                        'value' => function($model) {
                                            return '<span class="label label-' . $model->statusIv->label . '">' . $model->statusIv->name . '</span>';
                                        },
                                        'format' => 'raw',
                                        'contentOptions' => ['class' => 'text-center'],
                                        'headerOptions' => ['class' => 'text-center'],
                            ...
                            ?>
                                ...
                                <div class="x_panel">
                                    <div class="x_title">
                                        <h2>Status Temuduga</h2> 
                                        <div class="clearfix"></div>
                                    </div> 
                                    <div class="x_content">
                                        <?php foreach ($statusTemuduga as $status) { ?>
                                            <ul>
