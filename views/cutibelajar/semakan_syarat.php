...
<div class="x_panel">
                <div class="x_title">
                    ...
                    <h5> <strong><center>SEMAKAN SYARAT CUTI SABATIKAL | UNIT PENGAJIAN LANJUTAN (UPL)</center></strong> </h5>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-sm jambo_table table-bordered">
                            ...
                            { $no=0;?>
                                ...
                                <?php foreach ($sabatikal as $kpi) { $no++; 
                                $mod = \app\models\cbelajar\TblSemakSyarat::find()->where(['id' => $kpi->id, 'icno' => $icno, 'iklan_id'=>$kontrak->iklan_id])->one();
                                     
                                ?>
                                <tr>
                                    <td class="text-center"><?php echo $no; ?></td>
                                    <td class="text-justify"><?php echo $kpi->syarat; ?></td>
                                    <td class="text-center"><input type="radio" name="<?=$kpi->syarat_id.'semak_sabatikal'?>" value="y" <?php if($mod->semak_sabatikal){if($mod->semak_sabatikal === 'y'){echo "checked";}}?>></td>
                                    <td class="text-center"><input type="radio" name="<?=$kpi->syarat_id.'semak_sabatikal'?>" value="n" <?php if($mod->semak_sabatikal){if($mod->semak_sabatikal === 'n'){echo "checked";}}?>></td>
...
<div class="x_panel">
                <div class="x_title">
                    ...
                    <h5> <strong><center>SEMAKAN SYARAT LATIHAN INDUSTRI | UNIT PENGAJIAN LANJUTAN (UPL)</center></strong> </h5>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-sm jambo_table table-bordered">
                            ...
                            { $no=0;?>
                                ...
                                <?php foreach ($latihan as $latihan) { $no++; 
                                $mod = \app\models\cbelajar\TblSemakSyarat::find()->where(['id' => $latihan->id, 'icno' => $icno, 'iklan_id'=>$kontrak->iklan_id])->one();
                                     
                                ?>
                                <tr>
                                    <td class="text-center"><?php echo $no; ?></td>
                                    <td class="text-justify"><?php echo $latihan->syarat; ?></td>
                                    <td class="text-center"><input type="radio" name="<?=$latihan->syarat_id.'semak_latihan'?>" value="y" <?php if($mod->semak_latihan){if($mod->semak_latihan === 'y'){echo "checked";}}?>></td>
                                    <td class="text-center"><input type="radio" name="<?=$latihan->syarat_id.'semak_latihan'?>" value="n" <?php if($mod->semak_latihan){if($mod->semak_latihan === 'n'){echo "checked";}}?>></td>
