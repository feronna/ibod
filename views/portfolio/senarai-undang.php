...
<?php
  ...
  echo Html::a(Yii::t('app','SENARAI UNDANG-UNDANG'), ['/portfolio/senarai-undang','id' => $deskripsi->id], ['class' => 'btn btn-warning']);
 ...
 echo Html::a(Yii::t('app','JANA MYPORTFOLIO'), ['/portfolio/jana-portfolio','id' => $deskripsi->id], ['class' => 'btn btn-success']);
  ?>
     ...
     <div class="x_panel">
        ...
        <div class="x_title">
            <strong><h2>SENARAI UNDANG-UNDANG, PERATURAN DAN PUNCA KUASA</h2></strong>
        ...
        <div class="x_content">
                ...
                <label class="control-label col-md-3 col-sm-3 col-xs-12">Senarai Undang-undang, Peraturan dan Punca Kuasa<span class="required" style="color:red;">*</span>
                </label>
                ...
                ?>
                    <tr>
                            ...
                            <td> <?= $item->catatan?></td>
                              ...
                              <td> <?php if($item->file != NULL){ 
                           echo  Html::a(''  . $item->file, Url::to(Yii::$app->FileManager->DisplayFile($item->file), true), ['target' => '_blank', 'style' =>  'text-decoration: underline; color:green' ])  ;
                             }else{
                          
                       echo Html::a('Muat naik dokumen', ['kemaskini-senarai-undang', 'id' => $item->id,  'myjd_id' => $item->myjd_id], ['class'=>'btn btn-success btn-xs']);
                        

                             }
                             ?></td>
                                     <td width="50px" class="text-center"><?= Html::a('<i class="fa fa-pencil" aria-hidden="true"></i>', ['kemaskini-senarai-undang', 'id' => $item->id, 'myjd_id' => $item->myjd_id]) ?> | <?= Html::a('<i class="fa fa-trash" aria-hidden="true"></i>', ['delete-senarai-undang', 'id' => $item->id, 'myjd_id' => $item->myjd_id], [
                                  'data' => [
