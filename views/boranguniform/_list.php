...
<table class="table table-striped table-bordered jambo_table" >
                        <thead>
                            <th scope="col" colspan=17" width="30%" class="headings"><center>PAKAIAN SERAGAM<br>
                            <tr class="headings">
                            <th class="text-center">Bil</th>
                            <th class="text-center">Nama Pemohon</th>
                            <th class="text-center">Tarik Mohon</th> 
                            <th class="text-center">Status Pegawai BPG</th>  
                        ...
                        <?php foreach ($model as $list) { ?>
                            <tr>
                                ...
                                <td class="text-center"><?php echo $list->entrydate; ?> </td> 
                                 <?php if ($list->status_pp == 'DILULUSKAN') { ?>
                                ...
                                <?php if ($list->jeniskemudahan == '11') { ?>
                                <td style=" ">
                                 <?php if($list->isActive2 == '1' && $list->jeniskemudahan == '11'){?>
                                <div class="container" align="center">
                                <?= \yii\helpers\Html::a(' KELULUSAN BPG', ['boranguniform/slulus', 'id' => $list->id], ['class'=>'fa fa-download', 'target' => '_blank'])?></td>
                                <?php }?>   
                                 <?php }?> 
                                <?php if ($list->stat_bendahari == 'MENUNGGU TINDAKAN' || $list->stat_bendahari == 'DALAM PROSES BAYARAN') { ?>
                                <td class="text-center"><?= Html::a('<i class="fa fa-eye">', ["boranguniform/tindakan_bendahari", 'id' => $list->id]); ?></td>
                                <?php } else { ?><td class="text-center"><?= Html::a('<i class="fa fa-eye">', ["boranguniform/tindakan_bendahari", 'id' => $list->id]); ?></td>
                                <?php } ?>
