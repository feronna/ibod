...
$bhg = RefBahagian::findOne(['bahagian_id' => $kriteria[0]['bahagian_id']]);
...
<?= $this->render('_menuBorang', ['lppid' => $lpp->lpp_id, 'job_group' => $lpp->gredJawatan->job_group]); ?>

<div class="row">
    <div class="col-xs-12 col-md-12 col-lg-12"> 
        <div class="x_panel">
            <div class="x_title">
                <h2><strong><?= 'Bahagian '.$bhg->bahagian_id.' - '.ucwords(strtolower($bhg->bahagian)) ?></strong> (Wajaran <?= $mrkhBhg['markah_bahagian'] ?>%)</h2>
...
//, 'onfocus' => "this.value=''"
, 'disabled' => ($lpp->PPP_sah == 1) or ($lpp->PPK == \Yii::$app->user->identity->ICNO and is_null($lpp->PP_ALL))])->label(false) : 'PPP'; ?></td>
                                <td class="text-center"><?= (($lpp->PPK == \Yii::$app->user->identity->ICNO)) ? $form1->field($p, "[$ind]markah_PPK")->textInput(['id' => "[$ind]markah_PPP", 'class' => 'form-control', 'placeholder' => '0.0', 'style' => 'text-align:center', 'maxlength' => 2
//, 'onfocus' => "this.value=''"
, 'disabled' => ($lpp->PPK_sah == 1)  or ($lpp->PPP_sah == 0)])->label(false) : 'PPK'; ?></td>
                            </tr>
                            <?php 
                            $abc = $abc + 1;
                            }?>
