...
<table class="table table-striped table-bordered jambo_table" >
                        <thead>
                            <th scope="col" colspan=17" width="30%" class="headings"><center>TAMBANG BELAS EHSAN<br> 
                            <tr class="headings">
                    ...
                    <?php if ($model) { ?>
                        <?php foreach ($model as $list) { ?>
                            <tr>
                                ...
                                <?= \yii\helpers\Html::a(' RESIT TIKET', Yii::$app->FileManager->DisplayFile($list->dokumen_sokongan), ['class'=>'fa fa-download', 'target' => '_blank'])?><br> 
                                <?= \yii\helpers\Html::a(' SIJIL KEMATIAN', Yii::$app->FileManager->DisplayFile($list->dokumen_sokongan2), ['class'=>'fa fa-download', 'target' => '_blank'])?>
                                </td>  <?php }?>
                                
                                <td style=" ">
                                <?php if($list->isActive2 == '1' && $list->jeniskemudahan == '5'){?>
                                <div class="container" align="center">
                                <?= \yii\helpers\Html::a(' KELULUSAN KETUA BSM', ['borangehsan/slulus', 'id' => $list->id], ['class'=>'fa fa-download', 'target' => '_blank'])?></td>
                                <?php }?> 
                                 
                                <?php if ($list->stat_bendahari == 'MENUNGGU TINDAKAN' || $list->stat_bendahari == 'DALAM PROSES BAYARAN') { ?>
                                <td class="text-center"><?= Html::a('<i class="fa fa-eye">', ["borangehsan/tindakan_bendahari", 'id' => $list->id]); ?></td>
                                <?php } else { ?><td class="text-center"><?= Html::a('<i class="fa fa-eye">', ["borangehsan/tindakan_bendahari", 'id' => $list->id]); ?></td>
                                <?php } ?>
