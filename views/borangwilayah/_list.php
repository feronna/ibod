...
<table class="table table-striped table-bordered jambo_table" >
                        <thead>
                             <th scope="col" colspan=17" width="30%" class="headings"><center>WILAYAH ASAL<br>
                            <tr class="headings">
                    ...
                    <?php if ($model) { ?>
                        <?php foreach ($model as $list) { ?>
                            <tr>
                                ...
                                <?php if ($list->jeniskemudahan == '7') { ?>
                                <td style=" ">
                                 <?php if($list->isActive == '1' && $list->jeniskemudahan == '7'){?>
                                <div class="container" align="center">
                                <?= \yii\helpers\Html::a(' KELULUSAN KETUA BSM', ['borangwilayah/slulus', 'id' => $list->id], ['class'=>'fa fa-download', 'target' => '_blank'])?></td>
                                <?php }?>  
                                <?php }?>
                                
                                <?php if ($list->stat_bendahari == 'MENUNGGU TINDAKAN' || $list->stat_bendahari == 'DALAM PROSES BAYARAN') { ?>
                                <td class="text-center"><?= Html::a('<i class="fa fa-eye">', ["borangwilayah/tindakan_bendahari", 'id' => $list->id]); ?></td>
                                <?php } else { ?><td class="text-center"><?= Html::a('<i class="fa fa-eye">', ["borangwilayah/tindakan_bendahari", 'id' => $list->id]); ?></td>
                                <?php } ?>
