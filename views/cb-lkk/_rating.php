...
?> 
 ...
 <p align="right"><?= Html::a('Back', ['lkk/sv-phd?i='.$model->reportID.'&id='.$model->icno ], 
         ['class' => 'btn btn-primary btn-sm']) ?></p><?php }else {?>
             
      <p align="right"><?= Html::a('Back', ['lkk/sv-master?i='.$model->reportID.'&id='.$model->icno ], 
...
//                                 $mod = \app\models\cbelajar\TblPrestasi::find()->where(['id' => $dok->id, 'idLanjutan'=> 37, 'iklan_id'=>15])->one();
 ...
 <div class="x_panel">
                ...
                <table class="table table-sm table-bordered jambo_table table-striped"> 
                    ...
                    <tr>
                        <th class="col-md-3 col-sm-3 col-xs-12"><center>OVERALL PERFORMANCE</center></th>
                        <td>
                            <?php 
                      $c = \app\models\cbelajar\Rating::find()->where(['idLkk' => $i,'idKriteria'=>5])->one();
                     $b = \app\models\cbelajar\Rating::find()->where(['idLkk' => $i,'idKriteria'=>7])->one();
                     $a = \app\models\cbelajar\Rating::find()->where(['idLkk' => $i,'idKriteria'=>6])->one();
                     $d = \app\models\cbelajar\Rating::find()->where(['idLkk' => $i,'idKriteria'=>4])->one();
                     $e = \app\models\cbelajar\Rating::find()->where(['idLkk' => $i,'idKriteria'=>3])->one();
                     $f = \app\models\cbelajar\Rating::find()->where(['idLkk' => $i,'idKriteria'=>2])->one();
                      $g = \app\models\cbelajar\Rating::find()->where(['idLkk' =>$i,'idKriteria'=>1])->one();
...
//                                   $mod = \app\models\cbelajar\TblNilaiSyarat::find()->where(['syarat_id' => $dok->syarat_id, 'icno' => $icno, 'iklan_id'=>$kontrak->iklan_id])->one();
                                ...
                                <tr>
                                    ...
                                    <td class="text-justify"><?php echo $dok->kriteria; ?></td>
                                    <td colspan="10"> 
                                        <select id="komen" class="form-control" name="<?= $dok->id?>">
...
<!--                                        <select id="komen" class="form-control" name="<?= $dok->id?>">
                                          <option value="<?= $mod->p_komen?>">1</option>
 ...
 <div class="x_panel">
  ...
  <tr>
                ...
                <div class="col-md-9 col-sm-9col-xs-12 col-md-offset-3">
                    ...
                    <p align="right">    <?= Html::submitButton('Save', ['class' => 'btn btn-success',
                        'data' => ['confirm' => ' PLEASE MAKE SURE ALL THE FIELDS ARE FILLED IN CORRECTLY, BEFORE SUBMIT']]) ?></p>
