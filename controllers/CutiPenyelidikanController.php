...
class CutiPenyelidikanController extends \yii\web\Controller
{
...
//        $biasiswa = TblBiasiswa::find()->where(['icno' => $icno, 'idBorang' => 43])->all();
         $semak = RefSemakan::find()->where(['cb' => "penyelidikan"])->all();
$pengajian2 = TblPengajian::find()->where(['icno' => $ICNO, 'idPermohonan' => $id, 'idBorang' => 44, 'status' => [1, 9]])->one() ? TblPengajian::find()->where(['icno' => $ICNO, 'idPermohonan' => $id, 'idBorang' => 44, 'status' => [1, 9]])->one() : new TblPengajian();

                $rs_d = \app\models\elnpt\TblPenyelidikan2::find()->where(['IC' => $ICNO, 'ProjectID' => $pengajian2->research_id])->one();
        ...
        if($model->status_jfpiu == 'Diperakukan' || $model->status_jfpiu == 'Tidak Diperakukan'){
            ...
            $view = 'none'; $edit = '';}         
          if(\app\models\cbelajar\TblAdmin::find()->where( [ 'icno' => Yii::$app->user->getId() ] )->exists()){
        return $this->render('/cutibelajar/penyelidikan/_lihatpermohonan', [
          ...
          'rs_d'=>$rs_d,'pengajian2'=>$pengajian2
        ]);}
        else{
        Yii::$app->session->setFlash('alert', ['title' => 'Harap Maaf', 'type' => 'error', 'msg' => 'Anda tiada akses']);
    ...
    public function actionPaView($ICNO,$id, $takwim_id) {
        $model = $this->findModel($id);
        $pengajian = TblPengajian::find()->where(['idPermohonan'=>$id,'icno'=> $ICNO, 'iklan_id' => $takwim_id])->all();
        $biasiswa = TblBiasiswa::find()->where(['idBorang'=>44,'icno'=> $ICNO,'iklan_id'  => $takwim_id])->all();
        $dokumen = TblFilePemohon::find()->where(['uploaded_by'=> $ICNO, 'iklan_id'  => $takwim_id])->all();
        $senarai_dokumen = $this->findDokumen(1);
        $biodata = $this->findMaklumat($ICNO);
        $statuss = $this->findPerkhidmatanbyICNO();
...
//$biasiswa = TblBiasiswa::find()->where(['icno'=>$ICNO])->all();
          $dokumen = TblFilePemohon::findAll(['uploaded_by'=> $ICNO, 'iklan_id'  => $takwim_id,'idBorang' => 44]);
          $dokumen2 = \app\models\cbelajar\TblFileKpm::findAll(['uploaded_by' =>$ICNO,'iklan_id' => $takwim_id]);
          $dokumen3 = \app\models\cbelajar\TblFileLn::findAll(['uploaded_by' =>$ICNO,'iklan_id' => $takwim_id]);
$pengajian2 = TblPengajian::find()->where(['icno' => $ICNO, 'idPermohonan' => $id, 'idBorang' => 44, 'status' => [1, 9]])->one() ? TblPengajian::find()->where(['icno' => $ICNO, 'idPermohonan' => $id, 'idBorang' => 44, 'status' => [1, 9]])->one() : new TblPengajian();
                $rs_d = \app\models\elnpt\TblPenyelidikan2::find()->where(['IC' => $ICNO, 'ProjectID' => $pengajian2->research_id])->one();
             $content = $this->renderPartial('/cutibelajar/penyelidikan/cetak-borang', [ 'model' => $model,
              'biodata' => $biodata,
              'keluarga' => $biodata->keluarga,
              'biasiswa' => $biasiswa,
              'akademik' => $biodata->akademik,
              ...
              'dokumen2' => $dokumen2,
              'dokumen3' => $dokumen3,
             'pengajian' => $pengajian,'semak'=>$semak,'pengajian2'=>$pengajian2,'rs_d'=>$rs_d]);
...
//            'lapordiri' => $lapordiri,
    ...
    public function actionGambar($id) {
        ...
        return $this->render('/cutibelajar/penyelidikan/form_gambar', [
                    'model' => $model,
                    'iklan' => $iklan,
    ...
    public function actionMaklumatPeribadi($id)
    {
        ...
        return $this->render('/cutibelajar/penyelidikan/form_peribadi', 
            [ 
...
public function actionTindakanKj($ICNO,$id, $takwim_id) {
        $model = $this->findModel($id);
        $pengajian = TblPengajian::find()->where(['icno' => $ICNO, 'idPermohonan' => $id, 'idBorang' => 44, 'status' => [1, 9]])->all();
        $biasiswa = TblBiasiswa::find()->where(['idPermohonan'=>$id,'icno'=> $ICNO,'iklan_id'  => $takwim_id])->all();
        $dokumen = TblFilePemohon::find()->where(['uploaded_by'=> $ICNO, 'iklan_id'  => $takwim_id])->all();
        $senarai_dokumen = $this->findDokumen(1);
        $biodata = $this->findMaklumat($ICNO);
...
$senarai_dokumenkpm = $this->findDokumenKpm(1);
        ...
        return $this->render('/cutibelajar/penyelidikan/semak_maklumat', [
              ...
              'statuss' => $statuss, 
              'img' => $biodata->img,
              'statuss' => $statuss, 'rs_d'=>$rs_d,'pengajian2'=>$pengajian2,
              'bil' => '1',
              'edit' => $edit,
              'view' => $view,
                'semak'=>$semak,'senarai_dokumen' =>$senarai_dokumen,'ICNO'=>$ICNO,'iklan'=>$iklan,
...
//     
    ...
    public function actionMaklumatAkademik($id)
    {
        ...
        return $this->render('/cutibelajar/penyelidikan/form_akademik', 
        [ 
    ...
    public function actionMaklumatPengajian($id) {
        ...
        $pengajianTinggi = TblPengajian::findAll(['icno' =>$this->ICNO(),'iklan_id' => $id, 'idBorang' => 44,'status'=>9]);
        $iklan = $this->findIklanbyID($id);
        $icno = Yii::$app->user->getId();
        $sabatikal2= $this->findSabatikal();
        ...
        return $this->render('/cutibelajar/penyelidikan/form_pengajian', 
        [
                    'model' => $model,
                    'iklan' => $iklan,
...
//        $permohonan = $this->findPermohonanbyID($id);
        ...
        if ($model->load($post = Yii::$app->request->post())) {
            ...
            $model->modeStudy = "PENYELIDIKAN";
            $model->modeID;
            $cuti->icno = $icno;
            $cuti->jenis_cuti_id = 17;
            $cuti->status = "INCOMPLETE";
            $cuti->tempoh =  $model->totalDaysInc;
            $cuti->full_date = $model->full_dt;
            $cuti->start_date = $model->tarikh_mula;
            $cuti->end_date = $model->tarikh_tamat;
            $cuti->mohon_dt = new \yii\db\Expression('NOW()');
            $model->save(false);
            $cuti->save(false);
        ...
        return $this->renderAjax('/cutibelajar/penyelidikan/_formpengajian', [
                    'model' => $model,
                    'iklan' => $iklan,
            'icno'=>$icno
        ]);
    }
    public function actionTambahPengajian2($id,$ICNO,$takwim_id) {
        $icno = Yii::$app->user->getId();
        $model = new TblPengajian();
        $iklan = $this->findIklanByID($takwim_id);
                $biodata = $this->findBiodata();
...
//        $permohonan = $this->findPermohonanbyID($id);
        ...
        if ($model->load($post = Yii::$app->request->post())) {
            ...
            $model->tahun = date("Y");
            $model->created_dt = new \yii\db\Expression('NOW()');
            $model->gred = $model->kakitangan->jawatan->gred;
            $model->status_proses = "H";
            $model->tempoh =  $model->totalDaysInc;
            $model->idBorang=44;
            $model->status = 9; //dalam proses;
            $model->modeStudy = "PENYELIDIKAN";
            $model->modeID;
            $model->idPermohonan = $id;
            $cuti->icno = $icno;
            $cuti->jenis_cuti_id = 17;
            $cuti->status = "INCOMPLETE";
            $cuti->tempoh =  $model->totalDaysInc;
            ...
            $cuti->save(false);
                ...
                return $this->redirect(['semakan-syarat', 'id' => $id,'ICNO'=>$ICNO,'takwim_id'=>$takwim_id]);
            
        }
        return $this->renderAjax('/cutibelajar/penyelidikan/_formpengajians', [
                    'model' => $model,
                    'iklan' => $iklan,
            'icno'=>$icno,'ICNO'=>$ICNO
        ]);
...
//        var_dump('a');die;
       if ($model->load(Yii::$app->request->post())) {
        ...
        return $this->renderAjax('/cutibelajar/penyelidikan/form_gambar', [
                    'model' => $model,
                    'id' => $id, 'iklan'=>$iklan
        ]);
    }
...
//            'iklan' => $this->findIklanbyID($model->iklan_id),
   ...
   public function actionMaklumatBiasiswa($id)
    {   $request = Yii::$app->request;
        $sponsor = TblBiasiswa::find()->where(['icno' =>$this->ICNO(),'iklan_id' => $id, 'idBorang'=>44])->all();
        $icno = Yii::$app->user->getId();
        $iklan = $this->findIklanbyID($id);
        ...
        return $this->render('/cutibelajar/penyelidikan/form_biasiswa', [
                    'model' => $model,
                    'iklan' => $iklan,
                    'sponsor'=> $sponsor,
                    'searchModel' => $searchModel,
    ...
    public function actionMaklumatKeluarga($id)
    {
        ...
        $model = new TblPermohonan();
        $model->icno = $icno;
        return $this->render('/cutibelajar/penyelidikan/form_keluarga', 
            [ 
    ...
    public function actionTambahBiasiswa($id) {
        ...
        return $this->render('/cutibelajar/penyelidikan/_formbiasiswa', [
            'model' => $model,
            'iklan' => $iklan,
            'model2' => $model2,
    ...
    public function actionSenaraiDokumenDimuatnaik($id)
    {
        ...
        $dokumen2 = \app\models\cbelajar\TblFilePemohon::findAll(['uploaded_by' =>$this->ICNO(),'iklan_id' => $id, 'idBorang' => 44]);

        return $this->render('/cutibelajar/penyelidikan/form_dokumen', 
            [ 
...
//                $simpan->namafile = $file_path;
           ...
           //}
                        ...
                        return $this->renderAjax('/cutibelajar/penyelidikan/update-dokumen-ln', [
                        'model' => $model,
                            'iklan'=>$iklan
...
//                $simpan->namafile = $file_path;
           ...
           //}
                        ...
                        return $this->renderAjax('/cutibelajar/penyelidikan/update-dokumen-kpm', [
                        'model' => $model,
                            'iklan'=>$iklan
...
//                              $modelAddress->idBorang = 1;
        ...
        return $this->render('/cutibelajar/penyelidikan/_tajaanluarsm', [
            'model' => $model,
...
//            
//        }
        return $this->render('/cutibelajar/penyelidikan/_kpt', [
//            'model' => $model,
            'iklan' => $iklan,
            'model2' => $model2,
            'modelCustomer' => $modelCustomer,
   ...
   public function actionSenaraiDokumen($id)
    {
        ...
        $senarai_dokumenln = $this->findDokumenLn(2);
        $iklan = $this->findIklanbyID($id);
        return $this->render('/cutibelajar/penyelidikan/form_upload_dokumen', 
        [
            'senarai_dokumen' => $senarai_dokumen,
            'senarai_dokumenkpm' => $senarai_dokumenkpm,
            'senarai_dokumenln' => $senarai_dokumenln,
...
//                $simpan->namafile = $file_path;
           ...
           //}
                        ...
                        return $this->renderAjax('/cutibelajar/penyelidikan/update-dokumen', [
                        'model' => $model,
                            'iklan'=>$iklan
...
//            ]);
     ...
     public function findDokumen($status) 
    {
        $senarai_dokumen = new ActiveDataProvider([
            'query' => \app\models\cbelajar\TblDokumen::find()->where(['status' => $status,'kategori'=>10]),
...
//            $model->tarikh_mohon = NULL;
    ...
    protected function findAkademik() {
     ...
     public function actionPengakuanPemohon($id) {
        $icno = Yii::$app->user->getId();
        $pengajian = TblPengajian::find()->where(['icno' => $this->ICNO(), 'iklan_id' => $id, 'idBorang' => 44, 'status' => [1, 9]])->all();
        $biasiswa = TblBiasiswa::findAll(['icno' => $this->ICNO(), 'iklan_id' => $id, 'idBorang' => 44]);
        $status = TblPermohonan::findAll(['icno' => $icno]); //senarai status permohonan
        $model = TblPermohonan::find()->where(['icno' => $icno, 'iklan_id' => $id, 'idBorang' => 44])->one() ? TblPermohonan::find()->where(['icno' => $icno, 'iklan_id' => $id, 'idBorang' => 44])->one() : new TblPermohonan();
        $pengajian2 = TblPengajian::find()->where(['icno' => $icno, 'iklan_id' => $id, 'idBorang' => 44, 'status' => [1, 9]])->one() ? TblPengajian::find()->where(['icno' => $icno, 'iklan_id' => $id, 'idBorang' => 44, 'status' => [1, 9]])->one() : new TblPengajian();
        $sponsor2 = TblBiasiswa::find()->where(['icno' => $icno, 'iklan_id' => $id, 'idBorang' => 44])->all() ?
                TblBiasiswa::find()->where(['icno' => $icno, 'iklan_id' => $id, 'idBorang' => 44])->all() :
                new TblBiasiswa();
        $ums = TblBiasiswa::find()->where(['icno' => $icno, 'iklan_id' => $id, 'idBorang' => 44, 'jenisCd' => 2])->all() ?
                TblBiasiswa::find()->where(['icno' => $icno, 'iklan_id' => $id, 'idBorang' => 44, 'jenisCd' => 2])->all() :
                new TblBiasiswa();
        $sokongan = \app\models\cbelajar\TblFilePemohon::find()->where(['uploaded_by' => $this->ICNO(),
                    'iklan_id' => $id, 'idBorang' => 44])->all();
//        $doktoral = \app\models\cbelajar\TblDokumen::find()->WHERE(['kategori' => 7])->all();
        $senarai_dokumen = $this->findDokumen(1);
        $senarai_dokumenkpm = $this->findDokumenKpm(1);
        $senarai_dokumenln = $this->findDokumenLn(2);
        $dok = $this->findWajib(97);
          $doktoral = \app\models\cbelajar\TblDokumen::find()->WHERE(['kategori' => 1])->all();
        $wajib = TblFilePemohon::find()->where(['uploaded_by' => $icno, 'iklan_id' => $id, 'idBorang' => 44]);
        $check = TblpImage::findOne(['ICNO' => $this->ICNO(), 'iklan_id' => $id]);
//        $checkb = TblBiasiswa::find()->where(['icno' => $this->ICNO(), 'iklan_id' => $id,'idBorang'=> 44])->one();
                        $semak = RefSemakan::find()->where(['cb'=>"penyelidikan"])->all();
        $rs_d = \app\models\elnpt\TblPenyelidikan2::find()->where(['IC' => $icno, 'ProjectID' => $pengajian2->research_id])->one();
...
//                    }
        ...
        if (TblPermohonan::find()->where(['icno' => $icno, 'iklan_id' => $id, 'idBorang' => 44, 'status_proses' => "Selesai Permohonan"])->exists()) {
        ...
        if (($model->kakitangan->statLantikan == "1" && $model->kakitangan->jawatan->job_category == "1") || ($model->kakitangan->statLantikan == "3" && $model->kakitangan->jawatan->job_category == "1") ){
            ...
            return $this->render(
                            '/cutibelajar/penyelidikan/form_pengakuan', [
                        'iklan' => $iklan,
                        'model' => $model,
                        'img' => $biodata->img,
                                                        'cp' => $biodata->cutipenyelidikan,
     ...
     public function findDokumenKpm($status) {
       ...
       public function actionRingkasan($id) {
        ...
        $research = \app\models\elnpt\TblPenyelidikan2::find()->where(['ProjectID'=>$id])->one();

        return $this->renderAjax('/cutibelajar/penyelidikan/_ringkasan', [
                    'research' => $research,
            'id'=>$id
    ...
    public function actionSemakanSyarat($id, $ICNO, $takwim_id) {
        ...
        $akad = Tblpendidikan::find()->where(['ICNO' => $ICNO, 'HighestEduLevelCd' => 7])->one();
        $senarai_dokumen = $this->findDokumen(1);
        $doktoral = RefSemakan::find()->where(['cb' => "penyelidikan"])->all();
...
$pengajian2 = TblPengajian::find()->where(['icno' => $ICNO, 'idBorang' => 44, 'status' => [1, 9]])->one() ? TblPengajian::find()->where(['icno' => $ICNO, 'idBorang' => 44, 'status' => [1, 9]])->one() : new TblPengajian();
        ...
        return $this->render('/cutibelajar/penyelidikan/semakan_syarat', [
                    'kontrak' => $kontrak,
                    'doktoral' => $doktoral,
                    'message' => $message,
                    'icno' => $kontrak->icno,
                    ...
                    'akad' => $akad,
                    'senarai_dokumen' => $senarai_dokumen,'id'=>$id,'takwim_id'=>$takwim_id,'rs_d'=>$rs_d,'pengajian2'=>$pengajian2,
...
//            $mod->catatan_phd = Yii::$app->request->post($kpi->syarat_id . 'catatan_phd');
     ...
     public function actionViewSyarat($id, $ICNO,$takwim_id) {
        ...
        $kriteriakpi = RefSemakan::find()->where(['cb'=>"penyelidikan"])->all();
        $pengajian = TblPengajian::find()->where(['idPermohonan' => $id])->all();
        $studi = TblPengajian::find()->where(['icno'=>$ICNO, 'status'=>9])->one();
        $senarai_dokumen = $this->findDokumen(1);
        $kontrak = $this->findModel($id);
...
$pengajian2 = TblPengajian::find()->where(['icno' => $ICNO, 'idPermohonan' => $id, 'idBorang' => 44, 'status' => [1, 9]])->one() ? TblPengajian::find()->where(['icno' => $ICNO, 'idPermohonan' => $id, 'idBorang' => 44, 'status' => [1, 9]])->one() : new TblPengajian();
        ...
        return $this->render(
                        '/cutibelajar/penyelidikan/_syarat', [
                    'icno' => $kontrak->icno,
                    'id' => $kontrak->id,
                            'ICNO'=>$ICNO,
                    'edit' => $edit,
 ...
 'senarai_dokumen' => $senarai_dokumen,    'doktoral'=>$doktoral, 'pengajian2'=>$pengajian2,'rs_d'=>$rs_d        ]
        );
    }
...
//        var_dump('a');die;
        ...
        return $this->renderAjax('/cutibelajar/penyelidikan/_updates', [
            'model' => $model,
...
//        }
     ...
     public function actionGenerateSemakanSyarat($id, $ICNO) {
        ...
        $pdf = new Pdf([
            ...
            'mode' => Pdf::MODE_BLANK, // leaner size using standard fonts
            'destination' => Pdf::DEST_BROWSER,
            'content' => $this->renderPartial('/cutibelajar/penyelidikan/_laporansemakan', [
   ...
   public function actionLihatPermohonan($id) {
        ...
        $icno = Yii::$app->user->getId();
        $model->icno = $icno;
        $model2 = TblPermohonan::find()->where(['icno' => $icno, 'idBorang' => 44])->one();
        $model3 = TblPermohonan::find()->where(['icno' => $icno, 'idBorang' => 44])->one(); //senarai status permohonan
        //senarai status permohonan
        $biodata = $this->findBiodata();
        $pengajian = TblPengajian::find()->where(['icno' => $this->ICNO(), 'idPermohonan' => $id, 'idBorang' => 44, 'status' => [1, 9]])->all();
        $biasiswa = TblBiasiswa::find()->where(['icno' => $icno, 'idBorang' => 44])->all();
        $dokumen = TblFilePemohon::find()->where(['uploaded_by' => $icno, 'idBorang' => 44])->all();
                        $semak = RefSemakan::find()->where(['cb'=>"penyelidikan"])->all();
 $senarai_dokumen = $this->findDokumen(1);
        $senarai_dokumenkpm = $this->findDokumenKpm(1);
        $senarai_dokumenln = $this->findDokumenLn(2);
        ...
        if (($model->kakitangan->statLantikan == "1" && $model->kakitangan->jawatan->job_category == "1") ) {
            if (TblPermohonan::find()->where(['icno' => $icno, 'id' => $id, 'status_proses' => "Selesai Permohonan"])->exists()) {
                return $this->render(
                                '/cutibelajar/penyelidikan/lihatpermohonan', [
                            //              'iklan' => $iklan,
                            'model' => $model,
                            'img' => $biodata->img,
                            'biodata' => $biodata,
                            ...
                            'model3' => $model3,
                                  ...
                                  'senarai_dokumen' => $senarai_dokumen,
                                'senarai_dokumenkpm' => $senarai_dokumenkpm,
                                'senarai_dokumenln' => $senarai_dokumenln,'iklan'=>$iklan,'rs_d'=>$rs_d,'pengajian2'=>$pengajian2
                                ]
                );
            }
        } else {
...
//                              $modelAddress->idBorang = 1;
        ...
        return $this->render('/cutibelajar/penyelidikan/_biasiswaums', [
            'model' => $model,
            'modelsAddress' => (empty($modelsAddress)) ? [new TblBiasiswa] : $modelsAddress,
            'iklan' => $iklan,
    ...
    public function actionTanpatajaan($id) {
        ...
        return $this->render('/cutibelajar/penyelidikan/_tanpatajaan', [
            'model' => $model,
            'iklan' => $iklan,
            'model2' => $model2,
