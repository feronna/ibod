...
class TuntutanVisaController extends \yii\web\Controller {
    ...
    public function behaviors() {
        return [
            ...
            'access' => [
                ...
                'only' => [ 'lihat-permohonan-visa', 'tuntut', 'view-permohonan-visa', 'tuntut-tesis', 'tuntut-akhir',
                ...
                'rules' => [
                    ...
                    [
                        'actions' => [ 'lihat-permohonan-visa', 'tuntut', 'view-permohonan-visa',
...
//                           
                    ...
                    [
                        'actions' => ['senarai-borang',  'lihat-permohonan-visa', 'tuntut', 'view-permohonan-visa'],
                        'allow' => true,
                        'matchCallback' => function($rule, $action) {
...
//        $model->idBorang = "CBDN";
        ...
        if ($checkApplication->exists()) {
            ...
            return $this->redirect(['lihat-permohonan-visa', 'id' => $tesis->id]);
        }
        if ($file) {
            $fileapi = Yii::$app->FileManager->UploadFile($file->name, $file->tempName, '04', 'cb_visa');
            $filepath = $fileapi->file_name_hashcode;
        } else {
            $filepath = '';
        }
        if ($file2) {
            $fileapi = Yii::$app->FileManager->UploadFile($file2->name, $file2->tempName, '04', 'cb_visa');
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $ntf->title = 'Pengajian Lanjutan - Permohonan Tuntutan Visa';
            $ntf->content = "Permohonan Tuntutan Visa menunggu tindakan semakan anda." . Html::a('<i class="fa fa-arrow-right"></i>', ['cbadmin/senaraitindakantuntutan'], ['class' => 'btn btn-primary btn-sm']);
            $ntf->ntf_dt = date('Y-m-d H:i:s');
            $ntf->save(false);
            $this->notifikasi($model->icno, "Permohonan Tuntutan Visa anda telah dihantar untuk tindakan BSM. " . Html::a('<i class="fa fa-arrow-right"></i>', ['cutibelajar/permohonan-semasa'], ['class' => 'btn btn-primary btn-sm']));
            Yii::$app->session->setFlash('alert', ['title' => 'Berjaya', 'type' => 'success', 'msg' => 'Permohonan telah dihantar.']);
            return $this->redirect(['lihat-permohonan-visa', 'id' => $model->id]); //
        }
        return $this->render('/lapordiri/visa/_visa', [
                    'model' => $model,
//                    'lapor' => $lapor,
            'lapordiri'=>$lapordiri,
//              'tesis'=>$tesis,
    ...
    public function actionLihatPermohonanVisa($id) {
        $icno = Yii::$app->user->getId();
//        $lapor = $this->findLapordiri($icno);
        $lapordiri = TblLapordiri::find()->where(['icno'=>$icno])->orderBy(['dt_lapordiri'=> SORT_DESC])->one();
        ...
        if (\app\models\cbelajar\TblTuntut::find()->where(['id' => $id, 'status_borang' => "Selesai Permohonan"])->exists()) {
            ...
            return $this->render('/lapordiri/visa/_lihatpermohonan', [
                        'model' => $model,
//                        'lapor' => $lapor,
                        'edit' => $edit,
                        'view' => $view,
    ...
    public function actionViewTuntutanVisa($id) {
...
//                $this->notifikasi($model->icno, "Dokumen Permohonan Tuntutan Yura anda tidak lengkap. Sila muatnaik semula dokumen. " . " " . Html::a('<i class="fa fa-arrow-right"></i>', ['cutibelajar/permohonan-semasa'], ['class' => 'btn btn-primary btn-sm']));
                ...
                return $this->redirect(['view-tuntutan-visa', 'id' => $id]);
            }
            ...
            if (\app\models\cbelajar\TblAdmin::find()->where([ 'icno' => Yii::$app->user->getId()])->exists()) {
                return $this->render('/lapordiri/visa/viewtuntutan', [
                            'model' => $model,
                            'view' => $view,
                            'edit' => $edit,
                ]);
...
//              $model->pID= $pengajian->id;
        ...
        return $this->renderAjax('update-borang-visa', [
                    'model' => $model,

        ]);
    }

    public function actionCetakTuntutVisa($id) {
        $model = \app\models\cbelajar\TblTuntut::find()->where(['id' => $id, 'status_borang' => "Selesai Permohonan"])->one();
        $nd = \app\models\cbelajar\TblTuntut::find()->where(['id' => $id, 'j_tuntutan' => "VISA"])->all();
        $icno = $model->icno;

        $css = file_get_contents('./css/cetak.css');

        $content = $this->renderPartial('/lapordiri/visa/cetak-visa', [ 'model' => $model, 'nd' => $nd,
        ]);
        ...
        $pdf = new Pdf([
            ...
            'options' => ['title' => 'Perakuan Tuntutan VISA'],
            // call mPDF methods on the fly
            'methods' => [
                'SetHeader' => ['Perakuan Tuntutan VISA']
//                     'WriteHTML' => [$css, 1]
            ], // call mPDF methods on the fly
        ]);
