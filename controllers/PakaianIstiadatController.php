...
use Yii;
use app\models\kemudahan\Borangpakaian;
...
/**
 * PakaianIstiadatController implements the CRUD actions for Borangehsan model.
 */
class PakaianIstiadatController extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        if (($model = Borangpakaian::findOne($id)) !== null) {
            return $model;
        }
     ...
     public function actionForm_pakaian()
    {  
         
        $icno = Yii::$app->user->getId();  
        $peg_tadbir = TblAccess::findOne(['admin_post' => 1 ]); 
        $peg_bsm = TblAccess::findOne(['admin_post' => 2]);    
        $model = new Borangpakaian(); 
        $model->entry_date = date('Y-m-d H:i:s');
        $model->entry_type = 1;
        $model->icno = $icno;
        $checkOpen = Kemudahan::find()->where(['status' => 0, 'jeniskemudahan' => 10])->one();
        $check = Borangpakaian::find()->where('YEAR(entry_date) > YEAR(NOW() - INTERVAL 3 YEAR)')->andWhere(['mohon' => 1 ,'icno' => $icno]);
        if($check ->exists()){
             Yii::$app->session->setFlash('alert', ['title' => 'Harap Maaf', 'type' => 'error', 'msg' => 'Anda belum layak untuk memohon. Kemudahan bayaran pakaian ini diberi setiap tiga (3) tahun sekali.']);
        ...
        $checkApplication = Borangpakaian::find()->where(['status_semasa' => 0,'icno' => $icno]);
       ...
       if($model->load(Yii::$app->request->post())){
        ...
        $model->dokumen_sokongan2 = $filepath2;
            if ($model->save(false)) {
                 ...
                 $ntf->icno = $peg_tadbir->icno ; // peg  penyelia perjawatan
                            $ntf->title = 'Permohonan Kemudahan Atas Talian';
                            $ntf->content = "Permohonan Kemudahan Tuntutan Pakaian Istiadat menunggu tindakan semakan anda.".Html::a('<i class="fa fa-arrow-right"></i>', ['pakaian-istiadat/senaraitindakan'], ['class'=>'btn btn-primary btn-sm']);
                            $ntf->ntf_dt = date('Y-m-d H:i:s');
                            $ntf->save();
                     ...
                     $ntf2->icno = $peg_bsm->icno; // peg perjawatan
                            ...
                            $ntf2->content = "Permohonan Kemudahan Tuntutan Pakaian Istiadat  menunggu tindakan perakuan anda.".Html::a('<i class="fa fa-arrow-right"></i>', ['pakaian-istiadat/senaraitindakan'], ['class'=>'btn btn-primary btn-sm']);
        ...
        return $this->render('form_pakaian', [
            'model' => $model, 
            'bil' => 1,
    ...
    public function actionSenaraitindakan()
    {
        ...
        if(Refpegawai::find()->where( ['pembantu_tadbir' => $icno] )->exists()){
            ...
            $senarai = Borangpakaian::find()->where(['in', 'status_kj', $status])->andWhere(['jeniskemudahan' => 10])->orderBy(['entry_date' => SORT_DESC]);
    ...
    public function actionSemakan_pt($id)
    {
        ...
        $query = Borangpakaian::find()->where(['icno' => $model])->orderBy(['entry_date' => SORT_DESC]);
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            return $this->redirect(['pakaian-istiadat/senaraitindakan']);
            }
        }
    ...
    public function actionTindakan_bsm($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
           ...
           $ntf->icno =  $chief->chief; // Tindakan Kj
                            $ntf->title = 'Permohonan Kemudahan Atas Talian';
                            $ntf->content = "Permohonan Kemudahan Tuntutan Pakaian Istiadat menunggu tindakan Kelulusan anda.".Html::a('<i class="fa fa-arrow-right"></i>', ['pakaian-istiadat/senaraitindakan'], ['class'=>'btn btn-primary btn-sm']);
                            $ntf->ntf_dt = date('Y-m-d H:i:s');
                            $ntf->save(); 
     ...
     public function actionTindakan_kj($id)
    {
        ...
        $query = Borangpakaian::find()->where(['mohon' => 1])->andWhere(['icno' => $model])->orderBy(['entry_date' => SORT_DESC]);
        ...
        if ($model->load(Yii::$app->request->post())) {
           ...
           $ntf->icno = $model->icno;
                            $ntf->title = 'Permohonan Kemudahan Atas Talian';
                            $ntf->content = "Permohonan Kemudahan Tuntutan Pakaian Istiadat anda tidak diluluskan .".Html::a('<i class="fa fa-arrow-right"></i>', ['borang/senarai'], ['class'=>'btn btn-primary btn-sm']);
                            $ntf->ntf_dt = date('Y-m-d H:i:s');
                            $ntf->save();    
           ...
           return $this->redirect(['pakaian-istiadat/senaraitindakan']);
            ...
            $ntf2->icno = $peg_bsm->icno; 
                            $ntf2->title = 'Permohonan Kemudahan Atas Talian';
                            $ntf2->content = "Permohonan Kemudahan Tuntutan Pakaian Istiadat telah diluluskan oleh Ketua BSM kini menunggu tindakan daripada anda.".Html::a('<i class="fa fa-arrow-right"></i>', ['pakaian-istiadat/senaraiberjaya'], ['class'=>'btn btn-primary btn-sm']);
                            $ntf2->ntf_dt = date('Y-m-d H:i:s');
                            $ntf2->save();  
    ...
    public function actionSenaraiberjaya()
    {
        ...
        $status = ['DILULUSKAN']; 
        $models = Borangpakaian::find()->All();
        $query = Borangpakaian::find()->where([ 'status_kj' => $status, 'jeniskemudahan' => 10]) ->andWhere(['isActive2' => 2]);
                ...
                foreach ($selection as $id) {
                    ...
                    $pay->PAY_ELAUN_ID = $model->jeniskemudahan;
                     ...
                     $ntf->icno = $peg_bendahari->icno; // notification kpd bendahari
                            $ntf->title = 'Permohonan Kemudahan Atas Talian';
                            $ntf->content = "Permohonan Kemudahan Tuntutan Pakaian Istiadat menunggu tindakan anda untuk diproses.".Html::a('<i class="fa fa-arrow-right"></i>', ['pakaian-istiadat/senaraibendahari'], ['class'=>'btn btn-primary btn-sm']);;
                            $ntf->ntf_dt = date('Y-m-d H:i:s');
                            $ntf->save();
    ...
    public function actionSlulus($id)
        {   $css = file_get_contents('./css/kelulusan.css');
            ...
            $model = $this->findModel($id);
            $icno = Yii::$app->user->getId();   
            $facility = Borangpakaian::find()->where(['icno' => $model])->andWhere(['id' =>  $id])->one();
     ...
     public function actionSenaraibendahari()
    {
        ...
        $status = ['DILULUSKAN'];
        $model = Borangpakaian::find()->where(['in', 'status_kj', $status])->andWhere(['isActive2' => 1, 'jeniskemudahan' => 10])->orderBy(['entry_date' => SORT_DESC])->All();
    ...
    public function actionTindakan_bendahari($id)
    {
     ...
     public function actionLaporan( $tahun = null, $bulan = null) {
        ...
        if($bulan == 0 ){
         $query =  Borangpakaian::find()->where(['YEAR(entry_date)' => $year])->andWhere(['mohon' => 1])->orderBy(['entry_date' => SORT_ASC]);
           
        }else{
            $query =  Borangpakaian::find()->where(['MONTH(entry_date)' => $mth])->andWhere(['YEAR(entry_date)' => $year])->andWhere(['mohon' => 1])->orderBy(['entry_date' => SORT_ASC]);
        
        }
        $sum = $query->sum('jumlah');
        $statistik = Borangpakaian::find()->select(new \yii\db\Expression("MONTH(`entry_date`) AS BULAN, SUM(`jumlah`) AS JUMLAH"))->where(['YEAR(entry_date)' => $year])->groupBy('MONTH(`entry_date`)')->asArray()->all();
    ...
    public function actionReportpentadbiran($tahun = null, $bulan = null)
    {
        ...
        if($bulan == 0){
        $model = Borangpakaian::find() ->where(['YEAR(entry_date)' => $year])->andWhere(['mohon' => 1])->orderBy(['entry_date' => SORT_ASC])->all();
       
        }else{
        $model = Borangpakaian::find() ->where(['YEAR(entry_date)' => $year])->andWhere(['MONTH(entry_date)' => $mth])->andWhere(['mohon' => 1])->orderBy(['entry_date' => SORT_ASC])->all();
    ...
    public function actionTuntutan($id){
        ...
        $model = Borangpakaian::findOne(['id' => $id]);
    ...
    { 
       ...
       if($model->load(Yii::$app->request->post())){ 
        ...
        $ntf->icno = $peg_bsm->icno; // peg  penyelia perjawatan
                     $ntf->title = 'Permohonan Kemudahan Atas Talian';
                     $ntf->content = "Permohonan Kemudahan Tuntutan Pakaian Istiadat menunggu tindakan daripada anda.".Html::a('<i class="fa fa-arrow-right"></i>', ['boranglesen/senaraiberjaya'], ['class'=>'btn btn-primary btn-sm']);
                     $ntf->ntf_dt = date('Y-m-d H:i:s');
                     $ntf->save(); 
                            
        $model->save(false);
        Yii::$app->session->setFlash('alert', ['title' => 'Berjaya', 'type' => 'success', 'msg' => 'Permohonan telah dihantar.']);
        return $this->redirect(['pakaian-istiadat/new-entry']);
