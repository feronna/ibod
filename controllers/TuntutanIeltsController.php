...
class TuntutanIeltsController extends \yii\web\Controller {
    ...
    public function behaviors() {
        return [
            ...
            'access' => [
                ...
                'only' => [ 'lihat-permohonan-ielts', 'tuntut', 'view-permohonan-ielts', 'tuntut-tesis', 'tuntut-akhir',
                ...
                'rules' => [
                    ...
                    [
                        'actions' => [ 'lihat-permohonan-ielts', 'tuntut', 'view-permohonan-ielts',
...
//                           
                    ...
                    [
                        'actions' => ['senarai-borang',  'lihat-permohonan-ielts', 'tuntut', 'view-permohonan-ielts'],
                        'allow' => true,
                        'matchCallback' => function($rule, $action) {
...
//        $model->idBorang = "CBDN";
        ...
        if ($checkApplication->exists()) {
            ...
            return $this->redirect(['lihat-permohonan-ielts', 'id' => $tesis->id]);
        }
        if ($file) {
            $fileapi = Yii::$app->FileManager->UploadFile($file->name, $file->tempName, '04', 'cb_ielts');
            $filepath = $fileapi->file_name_hashcode;
        } else {
            $filepath = '';
        }
        if ($file2) {
            $fileapi = Yii::$app->FileManager->UploadFile($file2->name, $file2->tempName, '04', 'cb_ielts');
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $ntf->title = 'Pengajian Lanjutan - Permohonan Tuntutan Ielts';
            $ntf->content = "Permohonan Tuntutan Ielts menunggu tindakan semakan anda." . Html::a('<i class="fa fa-arrow-right"></i>', ['cbadmin/senaraitindakantuntutan'], ['class' => 'btn btn-primary btn-sm']);
            $ntf->ntf_dt = date('Y-m-d H:i:s');
            $ntf->save(false);
            $this->notifikasi($model->icno, "Permohonan Tuntutan Ielts anda telah dihantar untuk tindakan BSM. " . Html::a('<i class="fa fa-arrow-right"></i>', ['cutibelajar/permohonan-semasa'], ['class' => 'btn btn-primary btn-sm']));
            Yii::$app->session->setFlash('alert', ['title' => 'Berjaya', 'type' => 'success', 'msg' => 'Permohonan telah dihantar.']);
            return $this->redirect(['lihat-permohonan-ielts', 'id' => $model->id]); //
        }
        return $this->render('/lapordiri/ielts/_ielts', [
                    'model' => $model,
//                    'lapor' => $lapor,
            'lapordiri'=>$lapordiri,
//              'tesis'=>$tesis,
    ...
    public function actionLihatPermohonanIelts($id) {
        $icno = Yii::$app->user->getId();
//        $lapor = $this->findLapordiri($icno);
        $lapordiri = TblLapordiri::find()->where(['icno'=>$icno])->orderBy(['dt_lapordiri'=> SORT_DESC])->one();
        ...
        if (\app\models\cbelajar\TblTuntut::find()->where(['id' => $id, 'status_borang' => "Selesai Permohonan"])->exists()) {
            ...
            return $this->render('/lapordiri/ielts/_lihatpermohonan', [
                        'model' => $model,
//                        'lapor' => $lapor,
                        'edit' => $edit,
                        'view' => $view,
    ...
    public function actionViewTuntutanIelts($id) {
...
//                $this->notifikasi($model->icno, "Dokumen Permohonan Tuntutan Yura anda tidak lengkap. Sila muatnaik semula dokumen. " . " " . Html::a('<i class="fa fa-arrow-right"></i>', ['cutibelajar/permohonan-semasa'], ['class' => 'btn btn-primary btn-sm']));
                ...
                return $this->redirect(['view-tuntutan-ielts', 'id' => $id]);
            }
            ...
            if (\app\models\cbelajar\TblAdmin::find()->where([ 'icno' => Yii::$app->user->getId()])->exists()) {
                return $this->render('/lapordiri/ielts/viewtuntutan', [
                            'model' => $model,
                            'view' => $view,
                            'edit' => $edit,
                ]);
...
//              $model->pID= $pengajian->id;
    ...
    public function actionCetakTuntutIelts($id) {
        $model = \app\models\cbelajar\TblTuntut::find()->where(['id' => $id, 'status_borang' => "Selesai Permohonan"])->one();
        $nd = \app\models\cbelajar\TblTuntut::find()->where(['id' => $id, 'j_tuntutan' => "IELTS"])->all();
        $icno = $model->icno;

        $css = file_get_contents('./css/cetak.css');

        $content = $this->renderPartial('/lapordiri/ielts/cetak-ielts', [ 'model' => $model, 'nd' => $nd,
        ]);
        ...
        $pdf = new Pdf([
            ...
            'options' => ['title' => 'Perakuan Tuntutan IELTS'],
            // call mPDF methods on the fly
            'methods' => [
                'SetHeader' => ['Perakuan Tuntutan IELTS']
//                     'WriteHTML' => [$css, 1]
            ], // call mPDF methods on the fly
        ]);
