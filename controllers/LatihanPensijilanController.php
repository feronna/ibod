...
class LatihanPensijilanController extends \yii\web\Controller {
...
//          $pengajian = $this->findPengajian($ICNO);
        ...
        $semak = RefSemakan::find()->where(['cb' => "pensijilan"])->all();
        $gambar = TblpImage::find()->where(['icno' => $ICNO, 'iklan_id' => $takwim_id])->orderBy(['created_dt' => SORT_DESC])->one();

        $biasiswa = TblBiasiswa::findAll(['icno' => $ICNO, 'iklan_id' => $takwim_id]);
//$biasiswa = TblBiasiswa::find()->where(['icno'=>$ICNO])->all();
        $dokumen = TblFilePemohon::findAll(['uploaded_by' => $ICNO, 'iklan_id' => $takwim_id, 'idBorang' => 51]);
        $content = $this->renderPartial('/cutibelajar/pensijilan/cetak-borang', [ 'model' => $model,
            'biodata' => $biodata,
            ...
            'img' => $biodata->img,
            'dokumen' => $dokumen,
            'pengajian' => $pengajian, 'semak' => $semak, 'gambar' => $gambar]);
...
//                     'WriteHTML' => [$css, 1]
    ...
    public function actionAdminview($ICNO, $id, $takwim_id) {
        ...
        $pengajian = TblPengajian::find()->where(['idPermohonan' => $id, 'icno' => $ICNO, 'iklan_id' => $takwim_id])->all();
        $biasiswa = TblBiasiswa::find()->where(['idBorang' => 51, 'icno' => $ICNO, 'iklan_id' => $takwim_id])->all();
...
//        $biasiswa = TblBiasiswa::find()->where(['icno' => $icno, 'idBorang' => 43])->all();
        ...
        if (\app\models\cbelajar\TblAdmin::find()->where([ 'icno' => Yii::$app->user->getId()])->exists()) {
            return $this->render('/cutibelajar/pensijilan/_lihatpermohonan', [
                        ...
                        'khusus_ln' => $khusus_ln,'gambar'=>$gambar
            ]);
        } else {
            Yii::$app->session->setFlash('alert', ['title' => 'Harap Maaf', 'type' => 'error', 'msg' => 'Anda tiada akses']);
...
//            'lapordiri' => $lapordiri,
    ...
    public function actionGambar($id) {
        ...
        return $this->render('/cutibelajar/pensijilan/form_gambar', [
                    'model' => $model,
                    'iklan' => $iklan,
        ]);
    }
    ...
    public function actionMaklumatPeribadi($id) {
        ...
        return $this->render('/cutibelajar/pensijilan/form_peribadi', [
    ...
    public function actionTindakanKj($ICNO, $id, $takwim_id) {
        $model = $this->findModel($id);
        $pengajian = TblPengajian::find()->where(['idPermohonan' => $id, 'icno' => $ICNO, 'iklan_id' => $takwim_id])->all();
        $biasiswa = TblBiasiswa::find()->where(['icno' => $ICNO, 'iklan_id' => $takwim_id])->all();
        $dokumen = TblFilePemohon::find()->where(['uploaded_by' => $ICNO, 'iklan_id' => $takwim_id])->all();
        $senarai_dokumen = $this->findDokumen(1);
        $biodata = $this->findMaklumat($ICNO);
        $gambar = TblpImage::find()->where(['icno' => $ICNO, 'iklan_id' => $takwim_id])->orderBy(['created_dt' => SORT_DESC])->one();
...
//        $biasiswa = TblBiasiswa::find()->where(['icno' => $icno, 'idBorang' => 43])->all();
        ...
        if ($icno == $model->app_by) {
            return $this->render('/cutibelajar/pensijilan/semak_maklumat', [
...
//     
    ...
    public function actionMaklumatAkademik($id) {
        ...
        return $this->render('/cutibelajar/pensijilan/form_akademik', [
    ...
    public function actionMaklumatPengajian($id) {
        ...
        $pengajianTinggi = TblPengajian::findAll(['icno' => $this->ICNO(), 'iklan_id' => $id, 'idBorang' => 51, 'status' => 9]);
        $iklan = $this->findIklanbyID($id);
        $icno = Yii::$app->user->getId();
        $sabatikal2 = $this->findSabatikal();
        ...
        return $this->render('/cutibelajar/pensijilan/form_pengajian', [
                    'model' => $model,
                    'iklan' => $iklan,
...
//        $permohonan = $this->findPermohonanbyID($id);
        ...
        if ($model->load($post = Yii::$app->request->post())) {
            ...
            $model->HighestEduLevelCd = 213;
            $model->idBorang = 51;
            $model->status = 9; //dalam proses;
            $model->modeStudy = "SEPENUH";
            $model->modeID;
            $model->save(false);
        ...
        return $this->render('/cutibelajar/pensijilan/_formpengajian', [
                    'model' => $model,
                    'iklan' => $iklan,
        ]);
    }
    ...
    public function actionBadanList() {
        $out = [];
        if (isset($_POST['depdrop_parents'])) {
            $parents = $_POST['depdrop_parents'];
            if ($parents != null) {
                $cat_id = $parents[0];
                $out = \app\models\hronline\profesionalcert::find()->select
                                        (['id' => 'ProfCertCd', 'name' => 'ProfCertNm'])->
                                where(['ProfBodyCd' => $cat_id])->asArray()->all();
...
//        var_dump('a');die;
        ...
        return $this->renderAjax('/cutibelajar/pensijilan/form_gambar', [
                    'model' => $model,
                    'id' => $id, 'iklan' => $iklan
        ]);
    }
...
//            'iklan' => $this->findIklanbyID($model->iklan_id),
    ...
    public function actionMaklumatBiasiswa($id) {
        ...
        return $this->render('/cutibelajar/pensijilan/form_biasiswa', [
                    'model' => $model,
                    'iklan' => $iklan,
                    'sponsor' => $sponsor,
                    'searchModel' => $searchModel,
    ...
    public function actionMaklumatKeluarga($id) {
        ...
        $model = new TblPermohonan();
        $model->icno = $icno;
        return $this->render('/cutibelajar/pensijilan/form_keluarga', [
    ...
    public function actionTambahBiasiswa($id) {
        ...
        return $this->render('/cutibelajar/pensijilan/_formbiasiswa', [
                    'model' => $model,
                    'iklan' => $iklan,
                    'model2' => $model2,
    ...
    public function actionSenaraiDokumenDimuatnaik($id) {
        ...
        $dokumen2 = \app\models\cbelajar\TblFilePemohon::findAll(['uploaded_by' => $this->ICNO(), 'iklan_id' => $id, 'idBorang' => 51]);

        return $this->render('/cutibelajar/pensijilan/form_dokumen', [
...
//                $simpan->namafile = $file_path;
            ...
            return $this->renderAjax('/cutibelajar/pensijilan/update-dokumen-ln', [
                        'model' => $model,
                        'iklan' => $iklan
            ]);
...
//                $simpan->namafile = $file_path;
            ...
            return $this->renderAjax('/cutibelajar/pensijilan/update-dokumen-kpm', [
                        'model' => $model,
                        'iklan' => $iklan
            ]);
...
//                              $modelAddress->idBorang = 1;
        ...
        return $this->render('/cutibelajar/pensijilan/_tajaanluarsm', [
                    'model' => $model,
    ...
    public function actionSenaraiDokumen($id) {
        ...
        $senarai_dokumen = $this->findDokumen(1);
        $iklan = $this->findIklanbyID($id);
        return $this->render('/cutibelajar/pensijilan/form_upload_dokumen', [
...
//                $simpan->namafile = $file_path;
            ...
            return $this->renderAjax('/cutibelajar/pensijilan/update-dokumen', [
                        'model' => $model,
                        'iklan' => $iklan
            ]);
...
//            ]);
    ...
    public function findDokumen($status) {
        $senarai_dokumen = new ActiveDataProvider([
            'query' => \app\models\cbelajar\TblDokumen::find()->where(['status' => $status, 'kategori' => 13]),
            'pagination' => [
                'pageSize' => false,
            ],
        ]);
...
//            $model->tarikh_mohon = NULL;
    ...
    public function actionPengakuanPemohon($id) {
        $icno = Yii::$app->user->getId();
        $pengajian = TblPengajian::find()->where(['icno' => $this->ICNO(), 'iklan_id' => $id, 'idBorang' => 51, 'status' => [1, 9]])->all();
        $biasiswa = TblBiasiswa::findAll(['icno' => $this->ICNO(), 'iklan_id' => $id, 'idBorang' => 51]);
        $status = TblPermohonan::findAll(['icno' => $icno]); //senarai status permohonan
        $model = TblPermohonan::find()->where(['icno' => $icno, 'iklan_id' => $id, 'idBorang' => 51])->one() ? TblPermohonan::find()->where(['icno' => $icno, 'iklan_id' => $id, 'idBorang' => 51])->one() : new TblPermohonan();
        $pengajian2 = TblPengajian::find()->where(['icno' => $icno, 'iklan_id' => $id, 'idBorang' => 51, 'status' => [1, 9]])->one() ? TblPengajian::find()->where(['icno' => $icno, 'iklan_id' => $id, 'idBorang' => 51, 'status' => [1, 9]])->one() : new TblPengajian();
        $sponsor2 = TblBiasiswa::find()->where(['icno' => $icno, 'iklan_id' => $id, 'idBorang' => 51])->all() ?
                TblBiasiswa::find()->where(['icno' => $icno, 'iklan_id' => $id, 'idBorang' => 51])->all() :
                new TblBiasiswa();
        $ums = TblBiasiswa::find()->where(['icno' => $icno, 'iklan_id' => $id, 'idBorang' => 51, 'jenisCd' => 2])->all() ?
                TblBiasiswa::find()->where(['icno' => $icno, 'iklan_id' => $id, 'idBorang' => 51, 'jenisCd' => 2])->all() :
                new TblBiasiswa();
        $sokongan = \app\models\cbelajar\TblFilePemohon::find()->where(['uploaded_by' => $this->ICNO(),
                    'iklan_id' => $id, 'idBorang' => 51])->all();
//        $doktoral = \app\models\cbelajar\TblDokumen::find()->WHERE(['kategori' => 7])->all();
        $senarai_dokumen = $this->findDokumen(1);
        $dok = $this->findWajib(138);
        $doktoral = \app\models\cbelajar\TblDokumen::find()->WHERE(['kategori' => 1])->all();
        $wajib = TblFilePemohon::find()->where(['uploaded_by' => $icno, 'iklan_id' => $id, 'idBorang' => 51]);
        $check = TblpImage::findOne(['ICNO' => $this->ICNO(), 'iklan_id' => $id]);
//        $checkb = TblBiasiswa::find()->where(['icno' => $this->ICNO(), 'iklan_id' => $id, 'idBorang' => 51])->one();
        $semak = RefSemakan::find()->where(['cb' => "pensijilan"])->all();
        $spon = new TblBiasiswa();
        $gambar = TblpImage::find()->where(['icno' => $this->ICNO(), 'iklan_id' => $id])->orderBy(['created_dt' => SORT_DESC])->one();
...
//                if ($checkb) {
                if ($dok) {
                    ...
                    $spon->tahun = date("Y");
                    $spon->idBorang = 51;
                    $spon->HighestEduLevelCd = 213;
                    $spon->idPermohonan = $model->id;
                    $spon->icno = $pengajian2->icno;
                    $spon->iklan_id = $id;
                    $spon->jenisCd = 2;
                    $spon->nama_tajaan = "BIASISWA PENGURUSAN UMS";
                    $spon->BantuanCd = 14;
                    $spon->save(false);
//                        foreach ($wajib as $wajib) {
////                $wajib = new \stdClass();
...
//                }
        ...
        if (TblPermohonan::find()->where(['icno' => $icno, 'iklan_id' => $id, 'idBorang' => 51, 'status_proses' => "Selesai Permohonan"])->exists()) {
        ...
        if (($model->kakitangan->statLantikan == "1" && $model->kakitangan->jawatan->job_category == "2") || ($model->kakitangan->statLantikan == "3" && $model->kakitangan->jawatan->job_category == "2")) {
            ...
            return $this->render(
                            '/cutibelajar/pensijilan/form_pengakuan', [
                        'iklan' => $iklan,
                        'model' => $model,
                        'img' => $biodata->img,
                        'biodata' => $biodata,
                        ...
                        'icno' => $icno, 'semak' => $semak, 'gambar' => $gambar
                            ]
            );
        } else {
            Yii::$app->session->setFlash('alert', ['title' => 'Harap Maaf', 'type' => 'error', 'msg' => 'Anda tiada akses']);
    ...
    public function actionSemakanSyarat($id, $ICNO, $takwim_id) {
        $iklan = $this->findIklanbyID($takwim_id);
        $message = '';
        $pengajian = TblPengajian::find()->where(['icno' => $ICNO, 'iklan_id' => $takwim_id])->all();
        $akad = Tblpendidikan::find()->where(['ICNO' => $ICNO, 'HighestEduLevelCd' => 7])->one();
        $senarai_dokumen = $this->findDokumen(1);
        $studi = TblPengajian::find()->where(['icno' => $ICNO, 'iklan_id' => $takwim_id])->one();
        $biasiswa = TblBiasiswa::find()->where(['icno' => $ICNO, 'iklan_id' => $takwim_id])->all();
        $senarai_dokumenkpm = $this->findDokumenKpm(1);
        ...
        $gambar = TblpImage::find()->where(['icno' => $ICNO, 'iklan_id' => $takwim_id])->orderBy(['created_dt' => SORT_DESC])->one();
        $doktoral = RefSemakan::find()->where(['cb' => "pensijilan"])->all();
        $khusus = \app\models\cbelajar\RefSemakan::find()->where(['cb' => "sepenuh", 'jenis' => 'khusus'])->all();
        $khusus_ln = \app\models\cbelajar\RefSemakan::find()->where(['cb' => "sepenuh", 'jenis' => 'khusus_ln'])->all();
        if (Yii::$app->request->post()) {

            foreach ($doktoral as $kpi) {
        ...
        return $this->render('/cutibelajar/pensijilan/semakan_syarat', [
                    'kontrak' => $kontrak,
                    'message' => $message,
                    'icno' => $kontrak->icno,
                    'edit' => $edit,
                    ...
                    'gambar' => $gambar,
                    'studi' => $studi, 'ICNO' => $ICNO, 'biasiswa' => $biasiswa, 'doktoral' => $doktoral, 'khusus' => $khusus, 'khusus_ln' => $khusus_ln
        ]);
    }
...
//            $mod->catatan_phd = Yii::$app->request->post($kpi->syarat_id . 'catatan_phd');
    ...
    public function actionViewSyarat($id, $ICNO, $takwim_id) {
        ...
        $kriteriakpi = RefSemakan::find()->where(['cb' => "ir"])->all();
        ...
        return $this->render(
                        '/cutibelajar/pensijilan/_syarat', [
                    'icno' => $kontrak->icno,
                    'id' => $kontrak->id,
                    'edit' => $edit,
                    'view' => $view,
                    ...
                    'senarai_dokumen' => $senarai_dokumen, 'doktoral' => $doktoral]
        );
    }
...
//        var_dump($model);die;
        ...
        return $this->renderAjax('/cutibelajar/pensijilan/_updatess', [
                    'model' => $model,
...
//        var_dump('a');die;
        ...
        return $this->renderAjax('/cutibelajar/pensijilan/_updates', [
                    'model' => $model,
        ]);
    }
    ...
    public function actionUpdateSemakanSyarat($id, $ICNO, $takwim_id) {
        ...
        if (Yii::$app->request->post()) {
            foreach ($semak as $kpi) {
                $this->savekpi($takwim_id, $id, $kontrak->icno, $kpi);
            }
            foreach ($khusus as $kpi) {
                $this->savekpi($takwim_id, $id, $kontrak->icno, $kpi);
            }
            foreach ($khusus_ln as $kpi) {
                $this->savekpi($takwim_id, $id, $kontrak->icno, $kpi);
            }
...
//        }
    ...
    public function actionGenerateSemakanSyarat($id, $ICNO) {
        ...
        $biodata = $this->findMaklumat($ICNO);
        $kriteriakpi = RefSemakan::find()->where(['cb' => 'pensijilan'])->all();
        ...
        $pdf = new Pdf([
            ...
            'mode' => Pdf::MODE_BLANK, // leaner size using standard fonts
            'destination' => Pdf::DEST_BROWSER,
            'content' => $this->renderPartial('/cutibelajar/pensijilan/_laporansemakan', [
    ...
    public function actionLihatPermohonan($id) {
        ...
        $icno = Yii::$app->user->getId();
        $model->icno = $icno;
        $model2 = TblPermohonan::find()->where(['icno' => $icno, 'idBorang' => 51])->one();
        $model3 = TblPermohonan::find()->where(['icno' => $icno, 'idBorang' => 51])->one(); //senarai status permohonan
        //senarai status permohonan
        $biodata = $this->findBiodata();
        $model4 = $this->findModel($id);
        $gambar = TblpImage::find()->where(['icno' => $icno, 'iklan_id' => $model4->iklan_id])->orderBy(['created_dt' => SORT_DESC])->one();

        $pengajian = TblPengajian::findAll(['icno' => $icno, 'idPermohonan' => $id, 'idBorang' => 51]);
        $biasiswa = TblBiasiswa::find()->where(['icno' => $icno, 'idBorang' => 51])->all();
        $dokumen = TblFilePemohon::find()->where(['uploaded_by' => $icno, 'idBorang' => 51])->all();
        $semak = RefSemakan::find()->where(['cb' => "pensijilan"])->all();
        $khusus = \app\models\cbelajar\RefSemakan::find()->where(['cb' => "sepenuh", 'jenis' => 'khusus'])->all();
        $khusus_ln = \app\models\cbelajar\RefSemakan::find()->where(['cb' => "sepenuh", 'jenis' => 'khusus_ln'])->all();
        $senarai_dokumen = $this->findDokumen(1);
        ...
        if (($model->kakitangan->statLantikan == "1" && $model->kakitangan->jawatan->job_category == "2")) {
            if (TblPermohonan::find()->where(['icno' => $icno, 'id' => $id, 'status_proses' => "Selesai Permohonan"])->exists()) {
                return $this->render(
                                '/cutibelajar/pensijilan/lihatpermohonan', [
                            //              'iklan' => $iklan,
                            'model' => $model,
                            'img' => $biodata->img,
                            'biodata' => $biodata,
                            ...
                            'senarai_dokumen' => $senarai_dokumen,
                            'senarai_dokumenkpm' => $senarai_dokumenkpm,
                            'senarai_dokumenln' => $senarai_dokumenln, 'iklan' => $iklan,
                            'khusus' => $khusus, 'khusus_ln' => $khusus_ln, 'models' => $models, 'gambar' => $gambar
                                ]
                );
            }
        } else {
    ...
    public function actionKemaskiniUlasan($id, $ICNO) {
        ...
        return $this->renderAjax('/cutibelajar/pensijilan/update-ulasan', [
                    'model' => $model,
...
//                              $modelAddress->idBorang = 1;
        ...
        return $this->render('/cutibelajar/pensijilan/_biasiswaums', [
                    'model' => $model,
                    'modelsAddress' => (empty($modelsAddress)) ? [new TblBiasiswa] : $modelsAddress,
                    'iklan' => $iklan,
        ]);
    ...
    public function actionTanpatajaan($id) {
        ...
        return $this->render('/cutibelajar/pensijilan/_tanpatajaan', [
                    'model' => $model,
                    'iklan' => $iklan,
                    'model2' => $model2,
        ]);
