...
//use app\models\hronline\BadanProfesional;
/**
 * BorangyuranController implements the CRUD actions for Tblyuran model.
 */
class BorangyuranController extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    /**
     * Finds the Tblyuran model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Tblyuran the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Tblyuran::findOne($id)) !== null) {
            return $model;
        }
    ...
    public function actionMaklumatYuran()
    {  
        $icno = Yii::$app->user->getId();   
        $model = new Tblyuran();
//        $ProfBody = new BadanProfesional();
        $model->icno = $icno;
        $model->entry_type = 1;
        $checkApplication = Tblyuran::find()->where(['status_semasa' => 0,'icno' => $icno]);
        $checkOpen = Kemudahan::find()->where(['status' => 0, 'jeniskemudahan' => 2])->one();
        $checkExist = Tblyuran::find()->where(['mohon' => 1, 'icno' => $icno]);
        $gred = GredJawatan::find()->where(['id' => $model->kakitangan->gredJawatan])->andwhere(['<', 'gred_no', '41'])->all(); 
//        $association = new TblBadanProfesional(); 
        $association = TblBadanProfesional::find()->where(['ICNO' => $model->icno])->all();  
//        $fees = TblBadanProfesional::findAll(['ICNO' => $model->icno]);
  
        $fees = (new \yii\db\Query())   //showing table data tblprprofassoc
        ->select('ProfBodyCd, ProfBodyOther') 
        ->from(['hronline.tblprprofassoc']) 
        ->where(['ProfBodyCd' => $association, 'ICNO' => $model->icno])  
        ->all(); 
        
//        $depart = TblBadanProfesional::find()
//        ->join('translate')
//        ->select('TblBadanProfesional.ProfBodyCd, translate.ProfBody') 
//        ->asArray()
//        ->all();
        
        $depart = (new \yii\db\Query())
        ->select('d.ProfBodyCd, t.ProfBody')
        ->from(['d' => 'hronline.tblprprofassoc', 't' => 'hronline.professionalassociation'])
        ->where('d.ProfBodyCd = t.ProfBodyCd')
        ->all();
        
//        $fees = (new \yii\db\Query())    //showing data table professionalassociation
//        ->select('ProfBodyCd, ProfBody') 
//        ->from(['hronline.professionalassociation']) 
//        ->where(['ProfBodyCd' => $association]) 
//        ->all();
...
//        
//        
//        $fees1 = (new \yii\db\Query())
//                ->select("ProfBodyCd, ProfBodyOther")
//                ->from('hronline.tblprprofassoc')
//                ->where(['ProfBodyCd' => $association]);
        
//        $fees->union($fees1, false);//false is UNION, true is UNION ALL      
//        $sql = $fees->createCommand()->getRawSql();
//        $listFees = BadanProfesional::findBySql($sql);
        ...
        if($checkExist->exists()){
             Yii::$app->session->setFlash('alert', ['title' => 'Harap Maaf', 'type' => 'error', 'msg' => 'Kemudahan ini diberi hanya sekali sahaja sepanjang tempoh perkhidmatan dengan kerajaan.']);
            return $this->redirect(['borang/senarai']);
        }
        $checkApmt = $this->findBiodata($this->ICNO()); 

        if( $checkApmt->statLantikan == [1,3] || $gred){ // Permohonan hanya untuk Pegawai dalam Kumpulan Pengurusan Tertinggi  
        Yii::$app->session->setFlash('alert', ['title' => 'Harap Maaf', 'type' => 'error', 'msg' => 'Permohonan ini hanya layak dipohon bagi kakitangan berstatus tetap / kontrak, dan bagi Gred 41 ke atas sahaja.']);
            return $this->redirect(['kemudahan/index']);
        } 
       ...
       if($model->load(Yii::$app->request->post())){
        ...
        $model->dokumen_sokongan2 = $filepath2;
          if($model->jumlah > 4000 || $model->payment == 'Yuran Pendaftaraan'){  
                          
                Yii::$app->session->setFlash('alert', ['title' => 'Harap Maaf', 'type' => 'error', 'msg' => 'Tuntutan dibenarkan hanyalah untuk bayaran/yuran masuk sahaja tertakluk kepada had maksimum RM 4,000 atau amaun resit, mana yang terendah!' ]);
                return $this->redirect(['borangyuran/maklumat-yuran']);
           }
            ...
            if ($model->save(false)) {
                 ...
                 $ntf->icno = $peg_tadbir->icno; // peg  penyelia perjawatan
                            $ntf->title = 'Permohonan Kemudahan Atas Talian';
                            $ntf->content = "Permohonan Kemudahan Tuntutan Yuran Keahlian/ Badan Ikhtisas menunggu tindakan semakan anda.".Html::a('<i class="fa fa-arrow-right"></i>', ['borangyuran/senaraitindakan'], ['class'=>'btn btn-primary btn-sm']);
                            $ntf->ntf_dt = date('Y-m-d H:i:s');
                            $ntf->save();
                     ...
                     $ntf2->icno = $peg_bsm->icno; // peg perjawatan
                            ...
                            $ntf2->content = "Permohonan Kemudahan Tuntutan Yuran Keahlian/ Badan Ikhtisas menunggu tindakan perakuan anda.".Html::a('<i class="fa fa-arrow-right"></i>', ['borangyuran/senaraitindakan'], ['class'=>'btn btn-primary btn-sm']);
...
//            'ProfBody' => $ProfBody,
    ...
    public function actionSemakan_pt($id)
    {
        ...
        $query = Tblyuran::find()->where(['mohon' => 1]) ->andWhere(['icno' => $model])->orderBy(['entry_date' => SORT_DESC]); 
        ...
        if ($model->load(Yii::$app->request->post())) {
            if($model->status_pt == ''){
               Yii::$app->session->setFlash('alert', ['title' => 'Tidak Berjaya Dihantar', 'type' => 'error', 'msg' => 'Sila lengkapkan form']); 
                ...
                return $this->redirect(['borangyuran/senaraitindakan']);
                }
        }
    ...
    public function actionTindakan_bsm($id)
    {
        ...
        $query = Tblyuran::find()->where(['icno' => $model])->orderBy(['entry_date' => SORT_DESC]);
        ...
        if ($model->load(Yii::$app->request->post())) {
           ...
           $ntf->icno =  $chief->chief; // Tindakan Kj
                            $ntf->title = 'Permohonan Kemudahan Atas Talian';
                            $ntf->content = "Permohonan Kemudahan Tuntutan Yuran Keahlian/ Badan Ikhtisas menunggu tindakan Kelulusan anda.".Html::a('<i class="fa fa-arrow-right"></i>', ['borangyuran/senaraitindakan'], ['class'=>'btn btn-primary btn-sm']);
                            $ntf->ntf_dt = date('Y-m-d H:i:s');
                            $ntf->save(); 
    ...
    public function actionTindakan_kj($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
           ...
           $ntf->icno = $model->icno; // pemohon
                            $ntf->title = 'Permohonan Kemudahan Atas Talian';
                            $ntf->content = "Permohonan Kemudahan Tuntutan Yuran Keahlian/ Badan Ikhtisas anda tidak diluluskan".Html::a('<i class="fa fa-arrow-right"></i>', ['borang/senarai'], ['class'=>'btn btn-primary btn-sm']);
                            $ntf->ntf_dt = date('Y-m-d H:i:s');
                            $ntf->save();  
           ...
           return $this->redirect(['borangyuran/senaraitindakan']);
               ...
               $ntf2 =  new Notification();
                ...
                $ntf2->title = 'Permohonan Kemudahan Atas Talian';
                $ntf2->content = "Permohonan Kemudahan Tuntutan Yuran Keahlian/ Badan Ikhtisas telah diluluskan oleh Ketua BSM kini menunggu tindakan daripada anda.".Html::a('<i class="fa fa-arrow-right"></i>', ['borangyuran/senaraiberjaya'], ['class'=>'btn btn-primary btn-sm']);
                $ntf2->ntf_dt = date('Y-m-d H:i:s');
                $ntf2->save(); 
    ...
    public function actionSenaraiberjaya()
    {
        ...
        $status = ['DILULUSKAN']; 
        $models = Tblyuran::find()->All();
        $query = Tblyuran::find()->where([ 'status_kj' => $status]) ->andWhere(['isActive2' => 2]);
                ...
                foreach ($selection as $id) {
                    ...
                    $pay->PAY_ELAUN_ID = $model->jeniskemudahan;
                     ...
                     $ntf->icno = $peg_bendahari->icno; // notification kpd bendahari
                            $ntf->title = 'Permohonan Kemudahan Atas Talian';
                            $ntf->content = "Permohonan Kemudahan Tuntutan Yuran Keahlian/ Badan Ikhtisas menunggu tindakan anda untuk diproses.".Html::a('<i class="fa fa-arrow-right"></i>', ['borangyuran/senaraibendahari'], ['class'=>'btn btn-primary btn-sm']);;
                            $ntf->ntf_dt = date('Y-m-d H:i:s');
                            $ntf->save();
    ...
    public function actionSlulus($id)
        {   $css = file_get_contents('./css/kelulusan.css');
            ...
            $model = $this->findModel($id);
            $icno = Yii::$app->user->getId();   
            $facility = Tblyuran::find()->where(['icno' => $model])->andWhere(['id' =>  $id])->one();
    ...
    public function actionSenaraibendahari()
    {
        ...
        $status = ['DILULUSKAN'];
        $model = Tblyuran::find()->where(['in', 'status_kj', $status])->andWhere(['isActive2' => 1])->orderBy(['entry_date' => SORT_DESC])->All();
     ...
     public function actionTindakan_bendahari($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
           ...
           $model->save(false);
           return $this->redirect(['borangyuran/senaraibendahari']);
           }
        }
    ...
    public function actionLaporan( $tahun = null, $bulan = null) {
        ...
        if($bulan == 0 ){
         $query =  Tblyuran::find()->where(['YEAR(entry_date)' => $year])->andWhere(['mohon' => 1])->orderBy(['entry_date' => SORT_ASC]);
           
        }else{
            $query =  Tblyuran::find()->where(['MONTH(entry_date)' => $mth])->andWhere(['YEAR(entry_date)' => $year])->andWhere(['mohon' => 1])->orderBy(['entry_date' => SORT_ASC]);
        
        }
        $sum = $query->sum('jumlah');
        $statistik = Tblyuran::find()->select(new \yii\db\Expression("MONTH(`entry_date`) AS BULAN, SUM(`jumlah`) AS JUMLAH"))->where(['YEAR(entry_date)' => $year])->groupBy('MONTH(`entry_date`)')->asArray()->all();
    ...
    public function actionReportpentadbiran($tahun = null, $bulan = null)
    {
        ...
        if($bulan == 0){
        $model = Tblyuran::find() ->where(['YEAR(entry_date)' => $year])->andWhere(['mohon' => 1])->orderBy(['entry_date' => SORT_ASC])->all();
       
        }else{
        $model = Tblyuran::find() ->where(['YEAR(entry_date)' => $year])->andWhere(['MONTH(entry_date)' => $mth])->andWhere(['mohon' => 1])->orderBy(['entry_date' => SORT_ASC])->all();
    ...
    { 
       ...
       if($model->load(Yii::$app->request->post())){ 
        ...
        $ntf->icno = $peg_bsm->icno; // peg  penyelia perjawatan
                     $ntf->title = 'Permohonan Kemudahan Atas Talian';
                     $ntf->content = "Permohonan Kemudahan Tuntutan Yuran Keahlian/ Badan Ikhtisas menunggu tindakan daripada anda.".Html::a('<i class="fa fa-arrow-right"></i>', ['borangyuran/senaraiberjaya'], ['class'=>'btn btn-primary btn-sm']);
                     $ntf->ntf_dt = date('Y-m-d H:i:s');
                     $ntf->save(); 
                            
        $model->save(false);
        Yii::$app->session->setFlash('alert', ['title' => 'Berjaya', 'type' => 'success', 'msg' => 'Permohonan telah dihantar.']);
        return $this->redirect(['borangyuran/new-entry']);
