...
/**
 * BoranglesenController implements the CRUD actions for Borangehsan model.
 */
class BoranglesenController extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        if (($model = Boranglesen::findOne($id)) !== null) {
            return $model;
        }
    ...
    public function actionForm_lesen()
    {  
...
//        $facility = new Kemudahan();
        ...
        $model->icno = $icno;
        $checkApplication = Boranglesen::find()->where(['status_semasa' => 0,'icno' => $icno]);
//        $end = $facility->endDate;
//        $start = $facility->startDate;
//        $checkOpen = Kemudahan::find()->where(['or', "endDate >= '$end'", "startDate <= '$start'"])->andwhere(['status' => 0])->one();
...
//        if($checkApmt->statLantikan != [1,3] && $checkApmt->gredJawatan != 244 && $checkApmt->gredJawatan != 241){
//            Yii::$app->session->setFlash('alert', ['title' => 'Harap Maaf', 'type' => 'info', 'msg' => 'Permohonan ini hanya layak dipohon bagi kakitangan berstatus tetap dan kontrak, dan bagi kakitangan Skim H atau Pembantu Operasi sahaja.']);
//            return $this->redirect(['kemudahan/index']); 
//        } 
        ...
        $model->dokumen_sokongan2 = $filepath2;
            if ($model->save(false)) {
                 ...
                 $ntf->icno = $peg_tadbir->icno; // peg  penyelia perjawatan
                            $ntf->title = 'Permohonan Kemudahan Atas Talian';
                            $ntf->content = "Permohonan Kemudahan Tuntutan Lesen Memandu menunggu tindakan semakan anda.".Html::a('<i class="fa fa-arrow-right"></i>', ['boranglesen/senaraitindakan'], ['class'=>'btn btn-primary btn-sm']);
                            $ntf->ntf_dt = date('Y-m-d H:i:s');
                            $ntf->save();
                     ...
                     $ntf2->icno = $peg_bsm->icno; // peg perjawatan
                            ...
                            $ntf2->content = "Permohonan Kemudahan Tuntutan Lesen Memandu menunggu tindakan perakuan anda.".Html::a('<i class="fa fa-arrow-right"></i>', ['boranglesen/senaraitindakan'], ['class'=>'btn btn-primary btn-sm']);
                            $ntf2->ntf_dt = date('Y-m-d H:i:s');
                            $ntf2->save();
             } 
             $this->pendingtask($peg_tadbir->icno, 1);
             $this->pendingtask($peg_bsm->icno, 1);
...
//            'facility' => $facility,
     ...
     public function actionSenaraitindakan()
    {
        ...
        if(Refpegawai::find()->where( ['pembantu_tadbir' => $icno] )->exists()){
            ...
            $senarai = Boranglesen::find()->where(['in', 'status_kj', $status])->andWhere(['jeniskemudahan' => 8])->orderBy(['entry_date' => SORT_DESC]);
    ...
    public function actionSemakan_pt($id)
    {
        ...
        $query = Boranglesen::find()->where(['icno' => $model])->orderBy(['entry_date' => SORT_DESC]);
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            return $this->redirect(['boranglesen/senaraitindakan']);
            }
        }
    ...
    public function actionTindakan_bsm($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
           ...
           $ntf->icno =  $chief->chief; // Tindakan Kj
                            $ntf->title = 'Permohonan Kemudahan Atas Talian';
                            $ntf->content = "Permohonan Kemudahan Tuntutan Lesen Memandu menunggu tindakan Kelulusan anda.".Html::a('<i class="fa fa-arrow-right"></i>', ['boranglesen/senaraitindakan'], ['class'=>'btn btn-primary btn-sm']);
                            $ntf->ntf_dt = date('Y-m-d H:i:s');
                            $ntf->save(); 
    ...
    public function actionTindakan_kj($id)
    {
        ...
        $query = Boranglesen::find()->where(['mohon' => 1])->andWhere(['icno' => $model])->orderBy(['entry_date' => SORT_DESC]);
        ...
        if ($model->load(Yii::$app->request->post())) {
           ...
           $ntf->icno = $model->icno;// peg  penyelia perjawatan
                            $ntf->title = 'Permohonan Kemudahan Atas Talian';
                            $ntf->content = "Permohonan Kemudahan Tuntutan Lesen Memandu anda tidak diluluskan.".Html::a('<i class="fa fa-arrow-right"></i>', ['borang/senarai'], ['class'=>'btn btn-primary btn-sm']);
                            $ntf->ntf_dt = date('Y-m-d H:i:s');
                            $ntf->save(); 
           ...
           return $this->redirect(['boranglesen/senaraitindakan']);
            ...
            $ntf2->title = 'Permohonan Kemudahan Atas Talian';
            $ntf2->content = "Permohonan Kemudahan Tuntutan Lesen Memandu telah diluluskan oleh Ketua BSM kini menunggu tindakan daripada anda.".Html::a('<i class="fa fa-arrow-right"></i>', ['boranglesen/senaraiberjaya'], ['class'=>'btn btn-primary btn-sm']);
            $ntf2->ntf_dt = date('Y-m-d H:i:s');
            $ntf2->save();     
    ...
    public function actionSenaraiberjaya()
    {
        ...
        $status = ['DILULUSKAN']; 
        $models = Boranglesen::find()->All();
        $query = Boranglesen::find()->where([ 'status_kj' => $status, 'jeniskemudahan' => 8]) ->andWhere(['isActive2' => 2]);
            ...
            if (Yii::$app->request->post('simpan')){
                ...
                foreach ($selection as $id) {
                    ...
                    $pay->PAY_ELAUN_ID = $model->jeniskemudahan;
                            ...
                            $ntf->title = 'Permohonan Kemudahan Atas Talian';
                            $ntf->content = "Permohonan Kemudahan Tuntutan Lesen Memandu menunggu tindakan anda untuk diproses.".Html::a('<i class="fa fa-arrow-right"></i>', ['boranglesen/senaraibendahari'], ['class'=>'btn btn-primary btn-sm']);;
                            $ntf->ntf_dt = date('Y-m-d H:i:s');
                            $ntf->save();
     ...
     public function actionSlulus($id)
        {   $css = file_get_contents('./css/kelulusan.css');
            ...
            $model = $this->findModel($id);
            $icno = Yii::$app->user->getId();   
            $facility = Boranglesen::find()->where(['icno' => $model])->andWhere(['id' =>  $id])->one();
    ...
    public function actionSenaraibendahari()
    {
        ...
        $status = ['DILULUSKAN'];
        $model = Boranglesen::find()->where(['in', 'status_kj', $status])->andWhere(['isActive2' => 1, 'jeniskemudahan' => 8])->orderBy(['entry_date' => SORT_DESC])->All();
    ...
    public function actionTindakan_bendahari($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
           ...
           $model->save();
           Yii::$app->session->setFlash('alert', ['title' => 'Berjaya', 'type' => 'success', 'msg' => 'Berjaya Dihantar!']);
           return $this->redirect(['boranglesen/senaraibendahari']);
           }
        }
    ...
    public function actionLaporan( $tahun = null, $bulan = null) {
        ...
        if($bulan == 0 ){
         $query =  Boranglesen::find()->where(['YEAR(entry_date)' => $year])->andWhere(['mohon' => 1])->orderBy(['entry_date' => SORT_ASC]);
           
        }else{
            $query =  Boranglesen::find()->where(['MONTH(entry_date)' => $mth])->andWhere(['YEAR(entry_date)' => $year])->andWhere(['mohon' => 1])->orderBy(['entry_date' => SORT_ASC]);
        
        }
        $sum = $query->sum('jumlah');
        $statistik = Boranglesen::find()->select(new \yii\db\Expression("MONTH(`entry_date`) AS BULAN, SUM(`jumlah`) AS JUMLAH"))->where(['YEAR(entry_date)' => $year])->groupBy('MONTH(`entry_date`)')->asArray()->all();
    ...
    public function actionReportpentadbiran($tahun = null, $bulan = null)
    {
        ...
        if($bulan == 0){
        $model = Boranglesen::find() ->where(['YEAR(entry_date)' => $year])->andWhere(['mohon' => 1])->orderBy(['entry_date' => SORT_ASC])->all();
       
        }else{
        $model = Boranglesen::find() ->where(['YEAR(entry_date)' => $year])->andWhere(['MONTH(entry_date)' => $mth])->andWhere(['mohon' => 1])->orderBy(['entry_date' => SORT_ASC])->all();
    ...
    public function actionTuntutan($id){
        // $model = Boranglesen::find()->where(['icno' => $model])->andWhere(['id' =>  $id])->one();
        $model = Boranglesen::findOne(['id' => $id]);
        ...
        $pagecount = $mpdf->SetSourceFile('uploads/tuntutan.pdf');
            for ($i=1; $i<=$pagecount; $i++) {
                $import_page = $mpdf->ImportPage($i);
                $mpdf->UseTemplate($import_page);
                if($i==1){ //page1
                $mpdf->WriteHTML($this->renderPartial('borangtuntutan', ['model' => $model]));
                }
                if ($i < $pagecount){
                $mpdf->AddPage();}
            }
    ...
    { 
       ...
       if($model->load(Yii::$app->request->post())){ 
        ...
        if($file2){
                $fileapi = Yii::$app->FileManager->UploadFile($file2->name, $file2->tempName, '04', 'dokumen_sokongan');
                $filepath2 = $fileapi->file_name_hashcode;   
        }
        else{
            $filepath2 = '';
        ...
        $ntf->icno = $peg_bsm->icno; // peg  penyelia perjawatan
                     $ntf->title = 'Permohonan Kemudahan Atas Talian';
                     $ntf->content = "Permohonan Kemudahan Tuntutan Lesen Memandu menunggu tindakan daripada anda.".Html::a('<i class="fa fa-arrow-right"></i>', ['boranglesen/senaraiberjaya'], ['class'=>'btn btn-primary btn-sm']);
                     $ntf->ntf_dt = date('Y-m-d H:i:s');
                     $ntf->save(); 
                            
        $model->save(false);
        Yii::$app->session->setFlash('alert', ['title' => 'Berjaya', 'type' => 'success', 'msg' => 'Permohonan telah dihantar.']);
        return $this->redirect(['boranglesen/new-entry']);
