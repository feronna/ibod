...
use app\models\kemudahan\Borangpasport;
...
/**
 * BorangpasportController implements the CRUD actions for Borangehsan model.
 */
class BorangpasportController extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        if (($model = Borangpasport::findOne($id)) !== null) {
            return $model;
        }
    ...
    public function actionForm_pasport()
    {  
         
        $icno = Yii::$app->user->getId();  
        $peg_tadbir = TblAccess::findOne(['admin_post' => 1 ]); 
        $peg_bsm = TblAccess::findOne(['admin_post' => 2]); 
        $model = new Borangpasport(); 
        $model->entry_date = date('Y-m-d H:i:s');
        $model->icno = $icno;
        $checkOpen = Kemudahan::find()->where(['status' => 0, 'jeniskemudahan' => 9])->one();
        ...
        $checkApplication = Borangpasport::find()->where(['status_semasa' => 0,'icno' => $icno]);
        ...
        if($checkApmt->statLantikan != 1 && $checkApmt->statLantikan != 3){
            Yii::$app->session->setFlash('alert', ['title' => 'Harap Maaf', 'type' => 'error', 'msg' => 'Permohonan ini hanya layak dipohon bagi kakitangan berstatus tetap dan kontrak sahaja.']);
            return $this->redirect(['kemudahan/index']);
        } 
       ...
       if($model->load(Yii::$app->request->post())){
        ...
        $model->dokumen_sokongan2 = $filepath2;
            if ($model->save(false)) {
                 ...
                 $ntf->icno = $peg_tadbir->icno ; // peg  penyelia perjawatan
                            $ntf->title = 'Permohonan Kemudahan Atas Talian';
                            $ntf->content = "Permohonan Kemudahan Tuntutan Pasport menunggu tindakan semakan anda.".Html::a('<i class="fa fa-arrow-right"></i>', ['borangpasport/senaraitindakan'], ['class'=>'btn btn-primary btn-sm']);
                            $ntf->ntf_dt = date('Y-m-d H:i:s');
                            $ntf->save();
                     ...
                     $ntf2->icno = $peg_bsm->icno;  // peg perjawatan
                            ...
                            $ntf2->content = "Permohonan Kemudahan Tuntutan Pasport menunggu tindakan perakuan anda.".Html::a('<i class="fa fa-arrow-right"></i>', ['borangpasport/senaraitindakan'], ['class'=>'btn btn-primary btn-sm']);
        ...
        return $this->render('form_pasport', [
            'model' => $model, 
            'bil' => 1,
     ...
     public function actionSenaraitindakan()
    {
        ...
        if(Refpegawai::find()->where( ['pembantu_tadbir' => $icno] )->exists()){
            ...
            $senarai = Borangpasport::find()->where(['in', 'status_kj', $status])->andWhere(['jeniskemudahan' => 9])->orderBy(['entry_date' => SORT_DESC]);
    ...
    public function actionSemakan_pt($id)
    {
        ...
        $query = Borangpasport::find()->where(['icno' => $model])->orderBy(['entry_date' => SORT_DESC]);
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            return $this->redirect(['borangpasport/senaraitindakan']);
            }
        }
    ...
    public function actionTindakan_bsm($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
           ...
           $ntf->icno =  $chief->chief; // Tindakan Kj
                            $ntf->title = 'Permohonan Kemudahan Atas Talian';
                            $ntf->content = "Permohonan Kemudahan Tuntutan Pasport menunggu tindakan Kelulusan anda.".Html::a('<i class="fa fa-arrow-right"></i>', ['borangpasport/senaraitindakan'], ['class'=>'btn btn-primary btn-sm']);
                            $ntf->ntf_dt = date('Y-m-d H:i:s');
                            $ntf->save(); 
...
//            'searchModel' => $searchModel,
    ...
    public function actionTindakan_kj($id)
    {
        ...
        $query = Borangpasport::find()->where(['mohon' => 1])->andWhere(['icno' => $model])->orderBy(['entry_date' => SORT_DESC]);
        ...
        if ($model->load(Yii::$app->request->post())) {
           ...
           $ntf->title = 'Permohonan Kemudahan Atas Talian';
           $ntf->content = "Permohonan Kemudahan Tuntutan Pasport anda tidak diluluskan".Html::a('<i class="fa fa-arrow-right"></i>', ['borang/senarai'], ['class'=>'btn btn-primary btn-sm']);
           $ntf->ntf_dt = date('Y-m-d H:i:s');
           $ntf->save();      
           ...
           return $this->redirect(['borangpasport/senaraitindakan']);
            ...
            $ntf2->icno = $peg_bsm->icno; // peg  penyelia perjawatan
            $ntf2->title = 'Permohonan Kemudahan Atas Talian';
            $ntf2->content = "Permohonan Kemudahan Tuntutan Pasport telah diluluskan oleh Ketua BSM kini menunggu tindakan daripada anda.".Html::a('<i class="fa fa-arrow-right"></i>', ['borangpasport/senaraiberjaya'], ['class'=>'btn btn-primary btn-sm']);
            $ntf2->ntf_dt = date('Y-m-d H:i:s');
            $ntf2->save(); 
    ...
    public function actionSenaraiberjaya()
    {
        ...
        $status = ['DILULUSKAN']; 
        $models = Borangpasport::find()->All();
        $query = Borangpasport::find()->where([ 'status_kj' => $status, 'jeniskemudahan' => 9]) ->andWhere(['isActive2' => 2]);
                ...
                foreach ($selection as $id) {
                    ...
                    $pay->PAY_ELAUN_ID = $model->jeniskemudahan;
                     ...
                     $ntf->icno = $peg_bendahari->icno; // notification kpd bendahari
                            $ntf->title = 'Permohonan Kemudahan Atas Talian';
                            $ntf->content = "Permohonan Kemudahan Tuntutan Pasport menunggu tindakan anda untuk diproses.".Html::a('<i class="fa fa-arrow-right"></i>', ['borangpasport/senaraibendahari'], ['class'=>'btn btn-primary btn-sm']);;
                            $ntf->ntf_dt = date('Y-m-d H:i:s');
                            $ntf->save();
    ...
    public function actionSlulus($id)
        {   $css = file_get_contents('./css/kelulusan.css');
            ...
            $model = $this->findModel($id);
            $icno = Yii::$app->user->getId();   
            $facility = Borangpasport::find()->where(['icno' => $model])->andWhere(['id' =>  $id])->one();
    ...
    public function actionSenaraibendahari()
    {
        ...
        $status = ['DILULUSKAN'];
        $model = Borangpasport::find()->where(['in', 'status_kj', $status])->andWhere(['isActive2' => 1, 'jeniskemudahan' => 9])->orderBy(['entry_date' => SORT_DESC])->All();
    ...
    public function actionLaporan( $tahun = null, $bulan = null) {
        ...
        if($bulan == 0 ){
         $query =  Borangpasport::find()->where(['YEAR(entry_date)' => $year])->andWhere(['mohon' => 1])->orderBy(['entry_date' => SORT_ASC]);
           
        }else{
            $query =  Borangpasport::find()->where(['MONTH(entry_date)' => $mth])->andWhere(['YEAR(entry_date)' => $year])->andWhere(['mohon' => 1])->orderBy(['entry_date' => SORT_ASC]);
        
        }
        $sum = $query->sum('jumlah');
        $statistik = Borangpasport::find()->select(new \yii\db\Expression("MONTH(`entry_date`) AS BULAN, SUM(`jumlah`) AS JUMLAH"))->where(['YEAR(entry_date)' => $year])->groupBy('MONTH(`entry_date`)')->asArray()->all();
    ...
    public function actionReportpentadbiran($tahun = null, $bulan = null)
    {
        ...
        if($bulan == 0){
        $model = Borangpasport::find() ->where(['YEAR(entry_date)' => $year])->andWhere(['mohon' => 1])->orderBy(['entry_date' => SORT_ASC])->all();
       
        }else{
        $model = Borangpasport::find() ->where(['YEAR(entry_date)' => $year])->andWhere(['MONTH(entry_date)' => $mth])->andWhere(['mohon' => 1])->orderBy(['entry_date' => SORT_ASC])->all();
    ...
    public function actionTuntutan($id){
        ...
        $model = Borangpasport::findOne(['id' => $id]);
    ...
    { 
       ...
       if($model->load(Yii::$app->request->post())){ 
        ...
        if($file2){
                $fileapi = Yii::$app->FileManager->UploadFile($file2->name, $file2->tempName, '04', 'dokument_sokongan');
                $filepath2 = $fileapi->file_name_hashcode;   
        }
        else{
            $filepath2 = '';
        ...
        $ntf->icno = $peg_bsm->icno; // peg  penyelia perjawatan
                     $ntf->title = 'Permohonan Kemudahan Atas Talian';
                     $ntf->content = "Permohonan Kemudahan Tuntutan Lesen Memandu menunggu tindakan daripada anda.".Html::a('<i class="fa fa-arrow-right"></i>', ['borangpasport/senaraiberjaya'], ['class'=>'btn btn-primary btn-sm']);
                     $ntf->ntf_dt = date('Y-m-d H:i:s');
                     $ntf->save(); 
                            
        $model->save(false);
        Yii::$app->session->setFlash('alert', ['title' => 'Berjaya', 'type' => 'success', 'msg' => 'Permohonan telah dihantar.']);
        return $this->redirect(['borangpasport/new-entry']);
