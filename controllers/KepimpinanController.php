...
class KepimpinanController extends \yii\web\Controller
{
    ...
    {
        $query = GredJawatan::find()->joinWith('biodata.serviceStatus')->select('`id`,`nama`,`gred`,count(`tblprcobiodata`.`ICNO`) AS `_totalCount`')
    ...
    public function actionSenaraiKakitangan($k, $s, $p, $j)
    { //kumpulan, status lantikan, status perkhidmatan, gredjawatan
        ...
        $query = Tblprcobiodata::find()->joinWith(['jawatan', 'department'])->select('`ICNO`,`COOldID`,`CONm`,`department`.`fullname` AS _temp')->where(['`gredjawatan`.`job_category`' => $k])
            ...
            ->andWhere(['!=', 'Status', '06'])->orderBy(['CONm' => SORT_ASC]);
