...
class TuntutanInsuransController extends \yii\web\Controller {
    ...
    public function behaviors() {
        return [
            ...
            'access' => [
                ...
                'only' => [ 'lihat-permohonan-insurans', 'tuntut', 'view-permohonan-insurans', 'tuntut-tesis', 'tuntut-akhir',
                ...
                'rules' => [
                    ...
                    [
                        'actions' => [ 'lihat-permohonan-insurans', 'tuntut', 'view-permohonan-insurans',
...
//                           
                    ...
                    [
                        'actions' => ['senarai-borang',  'lihat-permohonan-insurans', 'tuntut', 'view-permohonan-insurans'],
                        'allow' => true,
                        'matchCallback' => function($rule, $action) {
...
//                           
    ...
    protected function notifikasi($icno, $content) {
        ...
        $ntf->icno = $icno;
        $ntf->title = 'Tuntutan Pengajian Lanjutan';
        $ntf->content = $content;
...
//                $tesis = \app\models\cbelajar\TblDokumen::find()->where(['jenisDokumen'=>5])->all();
        $tesis = \app\models\cbelajar\TblTuntut::find()->where(['idBorang' => 50, 'icno' => $icno, 'status_borang' => "Selesai Permohonan"])->one();
...
//        $model->idBorang = "CBDN";
        ...
        if ($checkApplication->exists()) {
            ...
            return $this->redirect(['lihat-permohonan-insurans', 'id' => $tesis->id]);
        }
        if ($file) {
            $fileapi = Yii::$app->FileManager->UploadFile($file->name, $file->tempName, '04', 'cb_insurans');
            $filepath = $fileapi->file_name_hashcode;
        } else {
            $filepath = '';
        }
        if ($file2) {
            $fileapi = Yii::$app->FileManager->UploadFile($file2->name, $file2->tempName, '04', 'cb_insurans');
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $ntf->title = 'Pengajian Lanjutan - Permohonan Tuntutan Insurans';
            $ntf->content = "Permohonan Tuntutan Insurans menunggu tindakan semakan anda." . Html::a('<i class="fa fa-arrow-right"></i>', ['cbadmin/senaraitindakantuntutan'], ['class' => 'btn btn-primary btn-sm']);
            $ntf->ntf_dt = date('Y-m-d H:i:s');
            $ntf->save(false);
            $this->notifikasi($model->icno, "Permohonan Tuntutan Insurans anda telah dihantar untuk tindakan BSM. " . Html::a('<i class="fa fa-arrow-right"></i>', ['cutibelajar/permohonan-semasa'], ['class' => 'btn btn-primary btn-sm']));
            Yii::$app->session->setFlash('alert', ['title' => 'Berjaya', 'type' => 'success', 'msg' => 'Permohonan telah dihantar.']);
            return $this->redirect(['lihat-permohonan-insurans', 'id' => $model->id]); //
        }
        return $this->render('/lapordiri/insurans/_insurans', [
                    'model' => $model,
//                    'lapor' => $lapor,
            'lapordiri'=>$lapordiri,
//              'tesis'=>$tesis,
    ...
    public function actionLihatPermohonanInsurans($id) {
        $icno = Yii::$app->user->getId();
//        $lapor = $this->findLapordiri($icno);
        $lapordiri = TblLapordiri::find()->where(['icno'=>$icno])->orderBy(['dt_lapordiri'=> SORT_DESC])->one();
        ...
        if (\app\models\cbelajar\TblTuntut::find()->where(['id' => $id, 'status_borang' => "Selesai Permohonan"])->exists()) {
            ...
            return $this->render('/lapordiri/insurans/_lihatpermohonan', [
                        'model' => $model,
//                        'lapor' => $lapor,
                        'edit' => $edit,
                        'view' => $view,
    ...
    public function actionViewTuntutanInsurans($id) {
...
//                $this->notifikasi($model->icno, "Dokumen Permohonan Tuntutan Yura anda tidak lengkap. Sila muatnaik semula dokumen. " . " " . Html::a('<i class="fa fa-arrow-right"></i>', ['cutibelajar/permohonan-semasa'], ['class' => 'btn btn-primary btn-sm']));
                ...
                return $this->redirect(['view-tuntutan-insurans', 'id' => $id]);
            }
            ...
            if (\app\models\cbelajar\TblAdmin::find()->where([ 'icno' => Yii::$app->user->getId()])->exists()) {
                return $this->render('/lapordiri/insurans/viewtuntutan', [
                            'model' => $model,
                            'view' => $view,
                            'edit' => $edit,
                ]);
...
//              $model->pID= $pengajian->id;
    ...
    public function actionCetakTuntutInsurans($id) {
        $model = \app\models\cbelajar\TblTuntut::find()->where(['id' => $id, 'status_borang' => "Selesai Permohonan"])->one();
        $nd = \app\models\cbelajar\TblTuntut::find()->where(['id' => $id, 'j_tuntutan' => "INSURANS"])->all();
        $icno = $model->icno;

        $css = file_get_contents('./css/cetak.css');

        $content = $this->renderPartial('/lapordiri/insurans/cetak-insurans', [ 'model' => $model, 'nd' => $nd,
        ]);
        ...
        $pdf = new Pdf([
            ...
            'options' => ['title' => 'Perakuan Tuntutan Insurans'],
            // call mPDF methods on the fly
            'methods' => [
                'SetHeader' => ['Perakuan Tuntutan Insurans']
//                     'WriteHTML' => [$css, 1]
            ], // call mPDF methods on the fly
        ]);
