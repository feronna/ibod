 ...
 */
class TblRekodOt extends \yii\db\ActiveRecord
{
    ...
    {
        return 'keselamatan.tbl_rekod_ot';
    }
    ...
    public function getStatusAllOt()
    {
    ...
    public function getMapBtnIn()
    {
        ...
        if ($this->ot_in_lat_lng) {
            return Html::button('Masuk', ['value' => Url::to("index.php?r=kehadiran/show_map&latlng=$this->ot_in_lat_lng"), 'class' => 'mapBtn btn btn-sm btn-success', 'id' => 'modalButton']);
        }
    ...
    public static function TotalWorkingPerMonth($month, $year)
    {
        ...
        foreach ($cuti_umum as $umum) {
            if (TblRekodOt::DisplayDay($umum->tarikh_cuti) == 'Sat' || TblRekodOt::DisplayDay($umum->tarikh_cuti) == 'Sun') {
                $val++;
            }
        }

        $working_day = 0;
        foreach (TblRekodOt::dayInMonth($month, $year) as $k => $v) {
            if ($v != 'Sat' && $v != 'Sun') {
                $working_day++;
            }
        }
    ...
    public static function getRm($icno, $tarikh, $format = null)
    {
        ...
        $ref = TblOt::find()->where(['tarikh' => $tarikh])->andWhere(['icno' => $icno])->one();
    ...
    public static function DisplayCuti($icno, $tarikh)
    {
        ...
        if (TblRekodOt::getRm($icno, $tarikh) == 'RM') {
            $val = '<span class="label label-success">' . 'RM' . '</span>';
        }
        if (TblRekodOt::getRm($icno, $tarikh) == 'CKA') {
            $val = '<span class="label label-succes">' . 'CKA' . '</span>';
        }
        if (TblRekodOt::getRm($icno, $tarikh) == 'CGKA') {
            $val = '<span class="label label-succes">' . 'CGKA' . '</span>';
        }
        if (TblRekodOt::getRm($icno, $tarikh) == 'CG') {
            $val = '<span class="label label-succes">' . 'CG' . '</span>';
        }
        return $val;
        //        var_dump($val);die;
    }
    ...
    public function checkAbsent($icno, $tarikh)
    {
        ...
        if ($staff) {
            $shift = TblOt::find()->where(['icno' => $icno, 'tarikh' => $tarikh])->one();
                    //    var_dump($shift->shift_id);die;'
            //klu wp id = REST day ID = 34
            if(!$shift){
                $v = TRUE;
                            }else
            if ($shift->shift_id == '10' || $shift->shift_id == '9' || $shift->shift_id == '8' || $shift->shift_id == '7' || $shift->shift_id == "1"|| $shift->shift_id == '11'||$shift->shift_id == '15' ) {
                $v = TRUE;
            }
        }
        //-------------------Check RESTDAY------------------//
    ...
    public static function DisplayHours($icno, $tarikh)
    {
        ...
        // returns all inactive customers
        $sql = 'SELECT * FROM keselamatan.tbl_rekod_ot WHERE icno=:icno AND tarikh=:tarikh AND time_in IS NOT NULL AND time_out IS NOT NULL';
        $model = TblRekodOt::findBySql($sql, [':icno' => $icno, 'tarikh' => $tarikh])->asArray()->one();
    ...
    public static function DisplayShift($icno, $tarikh)
    {
        ...
        if ($model) {
            ...
            $syif = RefShifts::find()->where(['id' => 40])->one();
            $val = $syif->jenis_shifts;
        }
        // var_dump($val);die;
    ...
    public static function countKetidakpatuhan($icno, $month, $year, $type)
    {
        ...
        if ($type == 1) {
            $sql = 'SELECT * FROM keselamatan.tbl_rekod_ot WHERE icno=:icno AND MONTH(tarikh)=:month AND status_in = "LATE_IN"';
        }

        if ($type == 2) {
            $sql = 'SELECT * FROM keselamatan.tbl_rekod_ot WHERE icno=:icno AND MONTH(tarikh)=:month AND status_out = "EARLY_OUT"';
        }

        if ($type == 3) {
            $sql = 'SELECT * FROM keselamatan.tbl_rekod_ot WHERE icno=:icno AND MONTH(tarikh)=:month AND incomplete = 1';
        }

        if ($type == 4) {
            $sql = 'SELECT * FROM keselamatan.tbl_rekod_ot WHERE icno=:icno AND MONTH(tarikh)=:month AND absent = 1';
        }

        if ($type == 5) {
            $sql = 'SELECT * FROM keselamatan.tbl_rekod_ot WHERE icno=:icno AND MONTH(tarikh)=:month AND external = 1';
        }

        $model = TblRekodOt::findBySql($sql, [':icno' => $icno, ':month' => $month])->all();
    ...
    public static function TotalWorkingHours($icno, $month, $type = 1)
    {
        ...
        $sql = 'SELECT * FROM keselamatan.tbl_rekod_ot WHERE icno=:icno AND MONTH(tarikh)=:month AND time_in IS NOT NULL AND time_out IS NOT NULL';
        $model = TblRekodOt::findBySql($sql, [':icno' => $icno, ':month' => $month])->all();
    ...
    public static function RemarkStatus($icno, $tarikh)
    {
        ...
        $model = TblRekodOt::find()->where(['icno' => $icno, 'tarikh' => $tarikh])->asArray()->one();
    ...
    public static function checkToday($icno, $tarikh)
    {
        ...
        $model = TblRekodOt::find()->where(['icno' => $icno, 'tarikh' => $tarikh])->one();
