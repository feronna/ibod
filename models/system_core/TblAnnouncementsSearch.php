<?php

namespace app\models\system_core;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\models\system_core\TblAnnouncements;

/**
 * TblAnnouncementsSearch represents the model behind the search form of `app\models\system_core\TblAnnouncements`.
 */
class TblAnnouncementsSearch extends TblAnnouncements {

    /**
     * {@inheritdoc}
     */
    public function rules() {
        return [
            [['id', 'crawler', 'status'], 'integer'],
            [['tag', 'title', 'content', 'start_dt', 'end_dt', 'create_by', 'create_dt', 'update_by', 'update_dt'], 'safe'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios() {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params) {
        $query = TblAnnouncements::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
//        $query->andFilterWhere([
//            'id' => $this->id,
//            'start_dt' => $this->start_dt,
//            'end_dt' => $this->end_dt,
//            'create_dt' => $this->create_dt,
//            'update_dt' => $this->update_dt,
//            'crawler' => $this->crawler,
//            'status' => $this->status,
//        ]);

//        $query->andFilterWhere([
//            'or',
//            ['like', 'profiles.first_name', $this->userFullName],
//            ['like', 'profiles.last_name', $this->userFullName],
//        ]);

        $query->andFilterWhere(['like', 'tag', $this->tag])
                ->andFilterWhere(['like', 'title', $this->title])
                ->andFilterWhere(['like', 'content', $this->content])
                ->andFilterWhere(['like', 'create_by', $this->create_by])
                ->andFilterWhere(['like', 'update_by', $this->update_by]);

        return $dataProvider;
    }

}
