...
/**
 * This is the model class for table "facility.tbl_pasport".
 *
 * @property int $id
 ...
 */
class Borangpasport extends \yii\db\ActiveRecord
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return 'utilities.fac_tbl_pasport';
    }
    ...
    public function perMonth($year,$mth) {
        ...
        return Borangpasport::find()->where(['YEAR(entry_date)' => $year])->andWhere(['MONTH(entry_date)' => $mth])->andWhere(['mohon' => 1])->count();
    }
    public function getTotal($year,$mth) {
        
        return Borangpasport::find()->where(['YEAR(entry_date)' => $year])->andWhere(['MONTH(entry_date)' => $mth])->andWhere(['mohon' => 1])->sum('jumlah');
    }
     public function getTotalYear($year) {
        
        return Borangpasport::find()->where(['YEAR(entry_date)' => $year])->andWhere(['mohon' => 1])->sum('jumlah');
    }
    public function getTotalCount($year,$mth) {
        
        return Borangpasport::find()->where(['YEAR(entry_date)' => $year])->andWhere(['mohon' => 1])->count();
    }
