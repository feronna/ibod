...
//    public $dept_id;
    ...
    public function search($params) {
        ...
        if($this->jenis == '3') {
            $query = $query->innerJoinWith('staffAkses', false, 'LEFT JOIN')->orderBy(['DeptID' => SORT_ASC, 'gredJawatan' => SORT_ASC]);
        }
        
        switch($this->akses) {
            case '1':
                $query = $query->joinWith('staffAkses', false, 'LEFT JOIN')->orderBy(['gredJawatan' => SORT_ASC]);
                break;
            case '2':
                $query = $query->innerJoinWith('staffAkses', false)->orderBy(['gredJawatan' => SORT_ASC]);
                break;
            case '3':
                $query = $query->joinWith('staffAkses', false, 'LEFT JOIN')->where(['is', 'StafAkses.ICNO', null])->orderBy(['gredJawatan' => SORT_ASC]);
                break;
            default:
                break;
        }
        ...
        $query->andFilterWhere(['=', 'hronline.tblprcobiodata.ICNO', $this->ICNO])
